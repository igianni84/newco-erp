...
- [x] 6.1 Full suite green on **SQLite and PG17** (the `normalizeActorId` cross-engine `===`); `vendor/bin/phpstan analyse` 0 errors (max); `vendor/bin/pint --test` clean; `openspec validate parties-producer-approval-sod --strict` green. Then run the GUIDE §2.7 semantic-verify pass before the close ritual (tracker RM-08 🔴→✅, `log.md`, `hot.md` — the close ritual proper, outside this list).
  - **Done 2026-07-06:** full suite **1975/1975** (10497 assn) on SQLite **and** PG17 (`pg` container, postgres:17.10, `newco_test` — the cross-engine `===` in `normalizeActorId` + the `actor_role` CHECK / partial-index / plpgsql triggers genuinely exercised); PHPStan max **0**; Pint `--test` clean; `openspec validate --strict` green. **Semantic-verify §2.7** (independent subagent) → **0 CRITICAL**; 1 WARNING (residual `parties-producer-lifecycle` overclaim in the ADR *References* bullet — the RM-09 "F3" residual class) **fixed in-place this task**; 1 SUGGESTION (console self-approval test asserts notification title not body — deferred, content proven separately). All 10 tasks `[x]` → `CHANGE_COMPLETE`; merge/archive/push are the human's §2.7 step.
