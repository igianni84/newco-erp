...
- **Composer quality aliases (2.1):** scripts reference the bare binary name (`"format": "pint"`, `"lint": "pint --test"`) — Composer prepends `vendor/bin` to PATH, so no `vendor/bin/` prefix or `@php` needed. They mirror the `CLAUDE.md` Quality Commands table verbatim (format = fix, lint = `--test` check-only). `composer analyse` (2.3) completes the set; the skeleton-provided `test` alias (`config:clear` w/ `@no_additional_args` → `php artisan test`) was confirmed + pinned in 2.2 (NOT re-added — it already existed). `tests/Feature/QualityToolingTest.php` now pins **format + lint + test** script keys + the pint preset (4 tests) so a later `composer require` (Filament 3.1, Boost 3.2) that rewrites `composer.json` can't silently drop them — re-run it after any `composer require`.
