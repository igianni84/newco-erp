...
- [x] 3.2 Console **renew-at-capacity** in `tests/Feature/Modules/OperatorPanel/Parties/ProfileLifecycleConsoleTest.php` (R4; operator-console — *Operator advances a Profile through its lifecycle*)
  ...
  - Full suite green both engines; typecheck; format
  > ⓘ 2026-07-09: the prescribed mutant reds 4, **3 pre-existing** (`ProfileRenewalCapacityGateTest`) — it proves the pin fires, not that it is needed. And the `title`+`status`+`body` trio is **not** three conjuncts: written as one exact-array `toBe` it is ONE assertion (no short-circuit) that also pins *exactly one toast sent*, and all three fields are **dominated** — stripping `->body(...)` from the shared `SurfacesDomainActions` kit reds 6 tests, 5 pre-existing, across four console files. A kit-level field cannot be blind. The pin's real content is the line the task never named: **`assertActionVisible('renew')` against a FULL Club** — a console-side capacity gate on the visibility predicate reds **exactly 1 of 2389** (every other page-driven renew runs uncapped). Also corrected the file docblock + the past-grace test's name, both of which claimed the past-grace reject was *"the sole UI-reachable reject"*. See progress.md § 3.2.
