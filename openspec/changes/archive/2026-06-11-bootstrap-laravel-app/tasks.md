...
- [x] 3.3 GitHub Actions `.github/workflows/ci.yml`: triggers on push + pull_request; PHP setup matching local minor version; composer cache; steps `vendor/bin/pint --test` → `vendor/bin/phpstan analyse` → `php artisan test` on SQLite; quality loop green locally before committing the workflow.
- [x] 3.4 Write `docs/development.md` (clone → composer install → env → migrate → serve; the five Quality Commands; how to run/monitor `./ralph.sh`; exact installed versions incl. Boost; link https://filamentphp.com/docs/llms.txt as the agent-facing Filament docs index) and update `docs/INDEX.md` row; re-run full quality commands one final time.
