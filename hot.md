...
- **RM-04 ✅ reviewed & pushed:** `autoLiftable()` unchanged (both new types fall through to operator-lift-only); mini-ADR `2026-07-01-adopt-dec-008-hold-types-8` + INDEX; migration CHECK derives from `cases()` (no ALTER); Module-E consumers unwired (seam). Tests: enum count/map/truth-table, lift-discipline (`HoldLifecycleTest` 3→5), registry/read-API 6→8, console prose.
- **Active next:** **RM-09** (reconcile identity-auth ADR erasure overclaim) → RM-10 (ClubCredit `Issued`→`Accrued`) → RM-24 (Product-Type immutability). All S, Round 1.
