...
> **Correction (2026-07-01 · RM-09).** The decision below is unchanged; two supporting clauses were factually overstated and are corrected in place — **not** superseded, because the auth decision itself did not change. The "First-party" bullet and Reasoning #1 originally said the GDPR customer-erasure was "already built in Module K" / "already works." What is actually built is the erasure **seam** in the event substrate ([[2026-06-12-event-substrate-and-audit-store]]): PII-free `domain_events`, the `audit_records` `before`/`after` redaction path behind the structural-immutability trigger, and Customer PII confined to the Module K module table (`email`/`name`/`phone`/`date_of_birth`). The customer PII-erasure **flow** — K J-9/J-9a: an anonymise action, `anonymised_at`, PII overwrite-in-place, and the Address entity — does **not** exist yet (grep-confirmed 2026-07-01: no anonymise action, no `anonymised_at`, no Address model; `CloseCustomer` documents `closed` ≠ anonymised as a deferred `parties-anonymisation` seam). It is tracked as **RM-01** (`docs/validation/Remediation_Tracker.md`); the two clauses become fully true when RM-01 lands. **Update (2026-07-03):** RM-01 has since shipped (`parties-anonymisation`, merged + archived `2026-07-02-parties-anonymisation`) — the anonymise action, `anonymised_at`, PII overwrite-in-place, the PII-free `CustomerAnonymised` event, and the Address entity now exist; the two clauses below are now **fully true** (the customer PII-erasure *flow* is built, not just its seam).
...
- Anchored to binding constraints, not taste: invariant #13 EU data-residency (`Architecture_v0.3-MVP.md §5.4` "primary stores in EU"); Module K as the authoritative party registry (`Module_K §4.1` — email globally unique; `§7` — a Customer is identity+eligibility, **never a login**); and the GDPR erasure *seam* already built in the event substrate ([[2026-06-12-event-substrate-and-audit-store]]) — PII-free `domain_events`, the `audit_records` `before`/`after` redaction path behind the structural-immutability trigger (`redactor` its documented PG-production principal), and Customer PII confined to the Module K module table where in-place erasure will operate. A first-party principal keeps **one** identity, in our EU Postgres, inside that erasure-*ready* machinery; the customer PII-erasure *flow* itself (anonymise action + `anonymised_at`, Address entity, K J-9/9a) is not yet built — it lands with RM-01 (`docs/validation/Remediation_Tracker.md`). *(See the Correction note at the top — this clause originally overstated the flow as "already built".)*
...
1. **One authoritative identity.** Module K is the party registry by spec; a first-party principal referencing the party by id keeps a single source of truth, in our EU Postgres — inside the erasure-*ready* substrate (PII-free events + the `audit_records` redaction seam), on the module tables where the customer PII-erasure flow will operate once RM-01 lands (see the Correction note; the earlier draft overstated this as "already built").
