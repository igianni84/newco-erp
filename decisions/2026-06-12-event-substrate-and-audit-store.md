...
- **`domain_events` is PII-free by design → absolute immutability.** Payloads carry ids and business data (amounts, quantities, states, dates), never names/emails/addresses — profile data lives in module tables where Module K's overwrite-in-place erasure already works. GDPR erasure never touches the event log. Financial events are doubly safe: ids+amounts only, and Italian fiscal retention is an Art. 17(3)(b) lawful basis overriding erasure.
