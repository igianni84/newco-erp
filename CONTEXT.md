...
- **`kyc` Hold coupling** → `parties-holds`. This slice records the KYC **state** only and places/lifts **no Hold**. Because KYC has no domain event (above), the future auto-place-on-`pending` / auto-lift-on-`verified` coupling is **within-module Action orchestration** (the KYC Action calls the Hold place/lift), not event-driven — the unified Hold registry (6 types, 3 scopes) and the DEC-181 read-API gate are `parties-holds`.
