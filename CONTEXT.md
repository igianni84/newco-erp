...
The separation-of-duties workflow over every commercial-impact transition (`reviewed → active`, `active → retired`, `retired → reviewed`): the actors performing the configured steps **must be distinct people — self-approval is never allowed** (the SoD floor, §4.2, BR-Lifecycle-1). The **role count** is operational configuration (`config('catalog.approval.role_count')` ∈ {2, 3}, default **3** — the full Creator → Reviewer → Approver, or a lighter two-step Creator → Approver; `feedback_prd_rr_approval`); the floor holds at any depth. A governance step **requires an authenticated operator** (`newco_ops` + non-null `actor_id`); a `system`/null actor is rejected. The **append-only audit trail is the system of record** for who performed each step (no per-entity governance columns); **rejection** keeps the entity in `reviewed` (actor + notes + decision recorded, edited in place, **no revert to `draft`** — §4.3, BR-Lifecycle-6). Activation is additionally refused while the entity is **review-stale** (canon MVP-DEC-019): its review-governed content has been *refused*, or *changed*, since the last review decision. Staleness is **derived from the audit trail, never a schema flag**, and the derivation is **verb-filtered**: among the entity's audit actions ending in one of the four review-freshness-relevant suffixes — `.submitted`, `.resubmitted`, `.rejected`, `.identity_updated` — the **latest wins**, and it leaves the entity review-stale iff it is `.rejected` (an un-remediated rejection) or `.identity_updated` (content edited but not re-reviewed). An explicit **re-submit** (`reviewed → reviewed`, audit-only) clears it, and a `draft` edit is cleared by the subsequent `submit`. Every other catalog audit verb is invisible to the derivation — hence the **verb-collision discipline**: no catalog audit verb may end with one of those four suffixes unless it is *meant* to participate in review freshness (`enrichment_updated` and `whitelist_updated` deliberately do not). The operator console mirrors the same predicate for the re-submit button's visibility.
_Avoid_: maker-checker (use Creator → Reviewer → Approver), `created_by`/`approved_by` columns (the audit trail is the SoR), revert-to-draft (rejection stays in `reviewed`), "rejection-pending" (the condition is **review-stale** — a rejection is only one of its two causes), a raw latest-audit-action read (the derivation is verb-filtered)

**Producer activation gate**:
The hard cross-module gate (§5.4, BR-Producer-1): a **Product Master** cannot reach `active` unless its **linked Producer is `active`**, checked at the `reviewed → active` moment against the **producer-state projection** — never a Module K table (invariant 10). Only `status = active` opens it: a `registered` producer (it exists, so a Master may be *created* under it) and a `retired` one both block, and a `producer_id` with **no** projection row at all is **fail-closed** (not gated open), never gated open by absence. The PRD's "and KYC-verified" conjunct is satisfied **transitively upstream** (`parties-compliance` tightens `ActivateProducer`) — a documented seam, so the KYC bit never enters Module 0's gate or the cross-module event contract.
...
Catalog's **first cross-module read model** and its **single source of producer knowledge** — `catalog_producer_states` (`producer_id`, `status` ∈ {`registered`, `active`, `retired`}, a per-producer `last_event_id` watermark, plus display denormalizations). Maintained by the codebase's **first registered `DomainEventConsumer`**, which consumes Module K's `ProducerCreated` → `registered`, `ProducerActivated` → `active` and `ProducerRetired` → `retired` (§14.5) **idempotently and order-tolerantly** (latest-wins on the watermark, so a redelivered `ProducerCreated` can never downgrade an `active` row). **Two rules read it at different granularities — existence ≠ activeness**: the *Producer activation gate* asks "is producer X `active`?" (only `active` opens it), while `CreateProductMaster` asks "does producer X **exist**?" (**any** row admits creation; no row → rejected before any write). Consuming `ProducerRetired` is **block-new, never cascade-retire** (existing `active` Masters are preserved); consuming `ProducerActivated` **never auto-activates** a queued Master (re-submission is operator-initiated); consuming `ProducerCreated` creates **no** Catalog entity and does **not** open the gate.
_Avoid_: a Producer mirror table (it holds the producer's projected state by id — not the Producer), a cross-module query at gate or creation time, treating `registered` as activatable (existence ≠ activeness), a second producer-knowledge source (e.g. a Parties read contract alongside the projection)
...
**Identity edit (re-versioning)**:
BR-Audit-1's correction path: review-governed **identity content** is edited **in place**, the entity's `version` increments in the same UPDATE, and one append-only audit row carries the changed fields' **before/after** (plus `version` before/after) — the superseded values are retrievable from the trail, so no version *rows* are ever persisted. Exactly two identity edits exist: a **Product Master**'s identity content (name, appellation, region, winery story — a name/appellation change re-checks the identity-uniqueness key against every other non-retired Master) and a **Composite SKU**'s ordered constituent set (*a Composite's composition **is** its identity*). Both record the audit verb `identity_updated` and **no domain event**. Per-state matrix: `draft` free · `reviewed` **re-arms review** (the entity is review-stale until an explicit re-submit) · `active` stays `active` (the FSM has no `active → reviewed` edge — this is the *correction* case, not a re-review) · `retired` rejected (reopen first). Implemented by the `CatalogContentEdit` mechanic, Module 0's **second** audit writer beside `LifecycleTransition`.
_Avoid_: version rows / a `*_versions` table (the audit trail is the history), `active → reviewed` (an edit never moves state), calling a Composite composition change "maintenance" (it re-versions and re-arms), a Filament Edit page (edits are modal header actions on the View page)

**Enrichment data**:
A Product Variant's **observational** content — at launch the translatable `tasting_notes` prose. Editing it is **maintenance, not identity**: `version` is untouched, the audit verb is `enrichment_updated` (invisible to review freshness — an enrichment edit **never re-arms review**), and it is allowed in `draft`/`reviewed`/`active` (rejected in `retired`). It is the one catalog content edit that **emits a domain event** — `EnrichmentDataUpdated` (`entity_type` `ProductVariant`, payload `{product_variant_id}`, PII-free: the prose lives in the audit before/after, never in the contract) — fired in the same transaction **only when the stored value actually changes**; an identical-value write is a silent **no-op** (no write, no audit row, no event). It is the 22nd catalog event and the only one **not** recorded by a lifecycle transition; its Module S consumer is a deferred seam.
_Avoid_: identity or quality content (enrichment is observational — it re-arms nothing), bumping `version` on an enrichment edit, carrying the tasting-note prose in the event payload

**Layer-1 case-configuration whitelist**:
The PIM-level admissibility rule (§3.3, §7.1): which **Case Configurations** a product may be sold in — declared **per (Product Variant, Format) pair**, i.e. "this product, *in this format*". Stored as an FK-integral pivot (`catalog_variant_case_whitelists`, unique on the triple), maintained by **replacing the pair's admitted set**. An **empty pair is permissive** (no rows ⇒ every Case Configuration admitted — the PRD default), and enforcement happens **only at Sellable SKU activation**: a SKU whose Case Configuration is not among its pair's admitted set cannot activate. The rule is **never retroactive** — shrinking a whitelist leaves already-`active` SKUs untouched (state, audit and event logs alike). Maintenance is **audit-only** (`whitelist_updated`, before/after CC-id sets): no `version` change, no domain event, no review re-arm; allowed in `draft`/`reviewed`/`active` (rejected in `retired`). It is **Layer 1** because two further admissibility layers sit downstream and are *not* PIM's: Module A's Layer 2 and Module S's Layer 3.
_Avoid_: a breakability flag (BR-RefData-2 — Case Configuration carries none; splitting is decided at Layer 2/3), a per-Variant format-blind whitelist, a retroactive sweep on reduction, a JSON column (the pivot is FK-integral)
...
**Agreement scope**:
The partition within which BR-K-Agreement-1 holds — **at most one `active` ProducerAgreement per scope** (§14.6 BR-K-Agreement-1 clause 1). Scope is the `(producer_id, club_id)` tuple, where a `NULL` `club_id` denotes the **distinct Producer-wide scope**: the single-active rule binds two agreements only *within* the same scope (§4.6.1) — a same-scope replacement **supersedes** the prior active. **Clause 2 — cross-shape mutual exclusion** (`parties-module-k-br-guards`, design D2): the Producer-wide and per-Club **shapes** are mutually exclusive on the same Producer at the same time — activating one shape while the other is `active` is **rejected** (`ProducerAgreementScopeConflict`), never superseded; two per-Club agreements on **different** Clubs (same shape, distinct scopes) MAY both be `active`. Scope tuple resolved by founder decision (2026-06-15); clause 2 restores the frozen rule (AC-K-BR-Agreement-1).
_Avoid_: a single per-Producer active-agreement uniqueness (two per-Club actives on different Clubs are legal — the tuple is the scope; a `NULL` `club_id` is its own scope, not a wildcard), Producer-wide + per-Club both active on one Producer (the pre-guard coexistence claim clause 2 inverts)

**Profile**:
A Customer's **membership in one Club** — the Netflix-style model where one Customer holds **many Profiles**, one per Club, with **no separate Membership entity** (the Profile *is* the membership). Born `applied`; carries a `state` (the nine-state §4.2.1 machine), nullable `tier` / `role`, and an `invited_by_customer_id` referral seam (a non-FK id). Uniqueness is **"one non-terminal Profile per (Customer, Club)"** — enforced by a partial unique index over non-terminal states, so a `rejected` / `cancelled` / `inactive` Profile never blocks a fresh one (a re-application after a decline inserts a fresh `applied` row). The **activation subset** of the FSM is implemented (`parties-membership-activation`): `Applied → Approved` / `Applied → Rejected` are the *Membership approval (audit-only)* pair, and `Approved → Active` is `ActivateProfile`. The **demand-side status subset** is now implemented too (`parties-membership-suspension`): `Active ↔ Suspended` (`SuspendProfile` / `ReactivateProfile`, state-preserving — AC-K-FSM-2a), `Active → Lapsed → Active` grace (`LapseProfile` → `ProfileExpired`; `RenewProfile` → `ProfileRenewed`, within the 30-day grace from `lapsed_at`, DEC-034), `Active | Lapsed → Cancelled` (`CancelProfile`, **audit-only**, stamps `cancellation_reason`) and `Active → Inactive` (`DeactivateProfile` → `ProfileInactive`). `Cancelled` / `Inactive` are **terminal soft-delete** (never hard-deleted; the partial index excludes them so a fresh application is admitted). The **`Applied → WaitingList` capacity path** is now implemented too (`parties-hero-package`): a Profile is born `waiting_list` when its target Club is at its Hero-Package capacity, and an approve at parity **diverts** an `applied` Profile there instead of activating it — `WaitingListJoined` at both entry points. A Profile leaves the waitlist **only** by a Producer's manual approve or decline; **nothing promotes it automatically, on any trigger**, not even a seat freed by attrition (canon MVP-DEC-011).
_Avoid_: Membership (no such entity — the Profile is it), subscription, the Account (a billing container, not a membership), a `ProfileLapsed` event (the lapse event is `ProfileExpired`), a `ProfileCancelled` event (cancellation is audit-only — the catalog names none), automatic FIFO waitlist promotion (conversion is always the Producer's manual approve)

**Onboarding activation gate**:
The **hard composite gate** `ActivateCustomer` enforces on the Customer `pending → active` transition (§4.1 / §7.1; AC-K-J-1 + AC-K-BR-Identity-3) — a conjunction of the four onboarding gates plus the KYC-cleared rider: `email_verified_at` set **∧** `tc_accepted_at` set **∧** `privacy_accepted_at` set (three **additive-nullable acceptance-timestamp** columns on `parties_customers`, born NULL, written by the deferred registration surface or an operator — no production setter in this slice) **∧** `sanctions_status = passed` **∧** KYC **cleared** (`KycStatus::clears()`, NULL = cleared per DEC-071) whenever `kyc_required`. The transition is **explicit** — `ActivateCustomer` is never auto-fired by a sanctions verdict or a KYC transition (the status FSM is independent of the KYC/sanctions FSMs — §9.4; AC-K-BR-Customer-1); a gate-unmet or wrong-from-state call raises a localized `IllegalCustomerTransition`. The **Hero-Package capacity ceiling** (§13) does not bear on this gate: it is a **Club-seat** ceiling counted over *Profiles*, and a Customer's `pending → active` consumes no Club seat — so `ActivateCustomer` is uncapped **by construction**, not by deferral. The seat gate lives on the seat-consuming Profile transitions (`parties-hero-package`).
...
The Profile `Applied | WaitingList → Approved` (`ApproveProfile`) and `Applied | WaitingList → Rejected` (`DeclineProfile`) operator Actions — **the one retained producer write** (L-PP / K-Q4; the producer-facing HTTP surface is deferred, the Actions are operator/console-invocable — DEC-083). Both are **audit-only**: §15.2 names **no** `ProfileApproved` / `ProfileRejected`, so — exactly as a KYC transition records no KYC event — the `state` write IS the audit record. The approve path's own conditional event is the **Originating-Club one-shot lock** (`OriginatingClubLocked`): on the Customer's **first-ever** approval across any Club, `ApproveProfile` sets `originating_club_id` to the approving Club and records the lock in the same transaction — **idempotent** (a later Club's approval finds it set → no write, no event) and **immutable**. The lock is **not** a standalone Action (`LockOriginatingClub` / `SetOriginatingClub` never exist — it is an in-transaction side-effect). Approval is **seat-gated** (`parties-hero-package`): with a *Club seat* free it drives through to `Active` atomically (co-recording `ProfileActivated`, and `OriginatingClubLocked` on a first-ever approval); at **capacity** it instead **diverts** the Profile to `WaitingList`, recording `WaitingListJoined` **alone** — no charge, no Originating-Club lock, no `ProfileActivated`. Approving an already-`WaitingList` Profile whose Club is *still* full has no transition left to make and **throws**.
_Avoid_: a `ProfileApproved` / `ProfileRejected` event (the catalog names none), a standalone Originating-Club lock Action, re-firing the lock on a second Club approval, treating a capacity-diverted approve as an error (it is a lawful outcome — only the *already-waitlisted* approve at parity throws)
...
- **Hero-Package Capacity Invariant** (§13) + the `Applied → WaitingList` path → **`parties-hero-package`** (**now landed** — see *Parties Hero-Package capacity event — payload contract* below). The seat gate fires only where a transition **newly consumes a seat**: `ApproveProfile` (from `applied` **and** from `waiting_list`) and `RenewProfile`'s grace re-activation. It deliberately does **not** fire on `ActivateProfile` — `Approved` is transient, the seat was decided one step earlier in the same transaction, and re-gating would count it twice — nor on `ReactivateProfile`, where a `suspended` Profile never freed its seat and re-gating would let a temporary Hold **evict** a member. Capacity is read through a K-owned port whose launch adapter is config-backed; **Module K stores no capacity value** (`AC-K-XM-20`). **Still carved out:** Module A's capacity-adjust surface (`AC-K-J-14` / `AC-K-J-15`), the **period rollover** behind canon's grandfathered renewal (`AC-K-J-15a` — a canon criterion the frozen `spec/` does not carry; we model no `valid_to` and no rollover Action), and the Hero-Package Offer SKU shape (`AC-K-XM-19` — a Module 0 / S surface).
...
**Event↔transition naming traps (verbatim §15.2 — invent no event).** The event names do **not** track the state names: `Active → Lapsed` records **`ProfileExpired`** (there is **no `ProfileLapsed`** — the state is `Lapsed`, the event is `ProfileExpired`); `Lapsed → Active` (grace) records **`ProfileRenewed`** (never `ProfileReactivated`); `Suspended → Active` records **`ProfileReactivated`** (**only** this edge — never the grace edge); `Active → Inactive` records `ProfileInactive`; `Active → Suspended` records `ProfileSuspended`. **Audit-only — record NO event:** `CancelProfile` (`→ Cancelled`; §15.2 names **no `ProfileCancelled`** — §15.7 defers the cancellation-signal shape) and **all three Account transitions** (`SuspendAccount` / `ReactivateAccount` / `CloseAccount`; §15 names no Account-family event). No name outside the eight above is recorded **by a status transition** — no `ProfileLapsed`, `ProfileCancelled`, `AccountSuspended`, `AccountClosed` or `CustomerSegmentChanged`. **`WaitingListJoined` *is* recorded**, but by neither event family: it belongs to the Hero-Package capacity path (`parties-hero-package`), fires on **entry into `WaitingList`**, and rides no status edge at all — see *Parties Hero-Package capacity event — payload contract* below.
...
- **Hero-Package Capacity Invariant** (§13) + the `Applied → WaitingList` path → **`parties-hero-package`** (**now landed**): of this slice's Actions only `RenewProfile` is seat-gated — a `lapsed` Profile has **left** the seat set, so its grace re-activation re-consumes a seat, and at parity it **throws** (canon draws no `Lapsed → WaitingList` edge, and diverting would burn the 30-day grace clock). `ReactivateProfile` is **never** gated: `Suspended` occupies its seat, so a restore re-consumes nothing. **Customer segments** (+ `CustomerSegmentChanged`) → **`parties-customer-segments`** (still deferred). **Anonymisation** (`closed` ≠ anonymised — AC-K-BR-Customer-2) → **`parties-anonymisation`** (**now landed**): `AnonymiseCustomer` overwrites the Customer PII + its Addresses' personal fields in place with deterministic id-keyed placeholders, stamps `anonymised_at` (**orthogonal to `status`** — a `closed` Customer stays `closed`), redacts the Customer's `audit_records` `before` / `after`, and records the PII-free `CustomerAnonymised`; idempotent, and blocked only by an active `compliance` Hold (canon MVP-DEC-015). The Filament/operator + producer surfaces over these Actions → **`parties-operator-console`** / portal.
...
### Parties Hero-Package capacity event — payload contract

The Hero-Package capacity invariant (implemented by `parties-hero-package`) records **one** event, and it is neither an activation nor a status event: `WaitingListJoined`, recorded through the platform `DomainEventRecorder` in the **same transaction** as the write, tagged module `parties`, with the `ActorContext`-resolved `actor_role` + `actor_id` and a **strictly PII-free** payload (entity ids + the post-write enum value — a Profile is a membership join, not a Party, and carries no personal data). It is a **root** event, like every other Parties event, and it fires at **both** `WaitingList` entry points — the birth at application (`CreateProfile`) and the divert at approval (`ApproveProfile`) — because its declared consumer (HubSpot's waitlist confirmation) owes the same confirmation to either applicant. **Leaving** the waitlist records no counterpart: there is no `WaitingListLeft`, and an approve of an already-`waiting_list` Profile whose Club is *still* full **throws** rather than re-recording a second `WaitingListJoined`.

| Event (`name`) | `entity_type` | Payload keys |
|---|---|---|
| `WaitingListJoined` | `Profile` | `profile_id`, `customer_id`, `club_id`, `state` |

**Key-naming note.** Unlike the `ProfileActivated` / `ProfileRenewed` transition payloads, which carry the Profile id alone, this one also names the `customer_id` and the `club_id`: the consumer has to know *who* joined *which* Club's waitlist, and an id is the only PII-free way to say it. `state` is the **post-write** enum `->value` (`waiting_list`).

_Source: spec/02-prd/Module_K_PRD_v0.3-MVP.md §13 (Hero-Package capacity), §15.6 (`WaitingListJoined`) · spec/03-acceptance/Module_K_Acceptance_v0.3-MVP.md AC-K-J-13 (the 51st approve waitlists), AC-K-EVT-11 (the event), AC-K-XM-20 (no capacity storage in Module K) · decisions/2026-07-09-hero-package-capacity-seat-set-and-waitinglist.md (the seat set, the `parties_clubs` row lock, no auto-promotion; canon MVP-DEC-011 / MVP-DEC-017 / MVP-DEC-020) · decisions/2026-06-12-event-substrate-and-audit-store.md (transactional outbox; PII-free payloads; root `correlation_id` = own `event_id`)._
...
**Hero Package**:
The producer's curated annual package whose price sets the club's membership fee for that year. Its Module-A Allocation `qty` is the Club's **membership capacity** — the ceiling the *Club seat* below is counted against.

**Club seat**:
The unit the Hero-Package capacity invariant counts. A Club's occupancy is **derived** from Profile state, never materialised: the **seat-occupying set is `Active` + `Suspended`** (canon MVP-DEC-017 — a suspension is a temporary restriction, not a departure, so the seat is never freed). `Applied`, `WaitingList` and `Approved` hold **no** seat — `Approved` because it is a transient pass-through inside the atomic approve instant, so counting it would refuse the very approval that reserved the seat. A transition is **seat-consuming** when it *enters* that set from outside it (`→ Active` at approval, at waitlist conversion, and at grace renewal from `Lapsed`); those are the transitions the gate guards, under a `parties_clubs` row lock taken **before** the count so same-Club approvals serialise. A seat freed by attrition stays free until a Producer approves someone into it.
_Avoid_: a seat entity or seat table (occupancy is a `count(*)` over Profile state), a capacity column on `parties_clubs` (`AC-K-XM-20` — Module K stores no capacity), counting `Approved` or `WaitingList` as occupying, re-checking capacity on `Suspended → Active`
