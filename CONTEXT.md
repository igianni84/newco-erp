...
- **Enhanced-KYC + cadence/AML detection automation** → deferred automation (roadmap #12). The `enhanced_kyc_flag` / `enhanced_kyc_at`, `next_rescreen_at` and `trigger_source` fields **ship**; the €10k-single / €50k-cumulative detection scan, the daily 12-month re-screen cadence job and the AML-threshold auto re-screen are **deferred** (no Module-S/E spend data at launch; manual-first — §9.5). The operator ad-hoc re-screen Action ships now.
...
- **Automatic Hold triggers** for `payment` / `fraud` / `compliance` / `credit`, and the finance-driven subtypes `STORAGE_PAYMENT_FAILED` (DEC-160) + `CHARGEBACK_REVIEW` (DEC-168) → **Module E** (Phase 6). Each trigger keys off a Module E/S event that does not exist yet; the registry is trigger-agnostic, so the six base types + the manual path + the lift discipline ship now.
- **Hold → `suspended` coupling** (demand-side, AC-K-FSM-9) → **`parties-membership-lifecycle`**. Placing a Hold records the Hold but performs no account-`status` transition.
...
- **GDPR right-to-erasure × active-Hold precedence** (regulatory vs non-regulatory partition, AC-K-J-9a / DEC-027) → the anonymisation change.
...
- **`MembershipFeePaid` listener** (the production trigger for `Approved → Active`) → **Module E** (Phase 6). Module E does not exist; `ActivateProfile` is the within-module writer, invoked by the free-club / operator path now — **no** Module-E event contract is fabricated. **Club Credit** (also fee-paid-coupled) → `club-credit`, an independent consumer.
...
- **Hold → `suspended` coupling**, `Active → Suspended | Lapsed | Cancelled | Inactive`, and Account-status transitions → **`parties-membership-suspension`**. **Customer segments** (Member / Waiting-list / Legacy + `CustomerSegmentChanged`) → **`parties-customer-segments`**.
...
**Hold**:
The unified, trigger-agnostic account-restriction primitive of Module K (`parties_holds`) — the compliance floor every transaction-initiation gate rests on (§4.8 "FLOOR"; AC-K-FSM-10). One registry-of-record table holds **every** Hold (DEC-168 — even the finance-triggered ones, when they land), each carrying a `hold_type`, a polymorphic `scope` (`scope_type` + `scope_id`), a two-state `status` (`active | lifted`), an optional placement `reason`, and placement + lift audit metadata (actor + moment, plus `lift_reason`). Born `active`; the **Action is the sole writer** (`PlaceHold` / `LiftHold`). Module K is **Hold-blind by design** (§9.3) — it is the registry + the read-API; the commercial-action enforcement is the consuming module's (Module S/C/E, deferred). Placing a Hold performs **no** account-`status` transition — the Hold → `suspended` escalation is the demand-side `parties-membership-lifecycle`'s, not this primitive's.
...
**Hold type**:
The trigger family of a Hold — a `HoldType` enum with **exactly six** values (§4.8, verbatim): `admin`, `kyc`, `payment`, `fraud`, `compliance`, `credit`. This closed set is the registry's controlled vocabulary; because the registry is **trigger-agnostic** (AC-K-MVP-2), a new automatic trigger reuses an existing type rather than coining a new one. At launch the **only wired automatic trigger** is the `kyc` coupling (see *KYC lifecycle*); the `payment` / `fraud` / `compliance` / `credit` placements arrive with their Module E/S signals. The finance-driven subtypes the PRD names elsewhere — `STORAGE_PAYMENT_FAILED` (DEC-160) and `CHARGEBACK_REVIEW` (DEC-168) — map onto the `payment` / `fraud` families when Module E lands; they are **not** separate enum values.
_Avoid_: chargeback / storage-non-payment as Hold *types* (finance subtypes that map to `fraud` / `payment`), inventing a seventh type for a new trigger
