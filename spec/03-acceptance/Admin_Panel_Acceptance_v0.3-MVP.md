...
| **AC-AP-PWB-4** | *(D23 — KEPT)* The Producer Portal read + full 7-section self-serve reporting is available at launch (reads A/S/E/K — sell-through, settlement projections, PO status, financial dashboards, membership/club state); the Producer Portal is read-only except the one write (AC-AP-PWB-1). | PRD §2.2; D23 | AUTO (read availability) + MIXED (7-section completeness) |
