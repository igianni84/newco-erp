...
| **AC-K-FSM-7** | Producer FSM traverses `draft → active → retired` with `ProducerCreated` / `ProducerActivated` / `ProducerRetired`. Activation gated on Producer KYC being **cleared** (`verified` or `not_required`); KYC `pending` or `rejected` blocks. | §4.4 + §15.4; BR-K-Producer-2 | AUTO — negative paths: attempt activation with KYC `pending` and with KYC `rejected`, assert rejection; positive paths: activation with KYC `verified` and with KYC `not_required`, assert admitted |
...
| **AC-K-EVT-8** | `ProducerCreated` on creation; `ProducerActivated` on `draft → active` (KYC cleared — `verified` or `not_required`); `ProducerRetired` on transition to `retired`. **(Event names K's own — unchanged by the cascade.)** | §15.4 | AUTO — covered by AC-K-FSM-7 |
