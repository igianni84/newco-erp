...
**Distribution for Module 0 v0.3-MVP: ~102 total criteria** — the v0.1 88 (86 AUTO / 1 MIXED / 1 HUMAN) **+ 12 generalisation criteria (11 AUTO / 1 MIXED)** + 1 (AC-0-BR-Identity-5, AUTO, MVP-DEC-023) + 1 (AC-0-BR-RefData-3, AUTO, MVP-DEC-025) → **99 AUTO / 2 MIXED / 1 HUMAN.** Paolo's hands-on load: **2 MIXED items + 1 end-to-end demo session.**
...
| **AC-0-J-7** | Reviewer or Approver rejects an entity; it stays in `reviewed` with rejection flag + notes; **a pending rejection blocks approval** (an Approver attempt on a flagged entity is rejected); Creator edits in place (no revert-to-draft) and **explicitly re-submits**, which clears the live flag and restarts from review; approval succeeds only after re-review by a distinct actor; **editing review-governed content of a `reviewed` entity re-arms review** (approval blocked until re-reviewed); full rejection history preserved in audit trail. | §4.3; BR-Lifecycle-1, BR-Lifecycle-6 | AUTO — drive a 2-rejection-round scenario; assert state stays `reviewed`, an Approver attempt on a flagged entity is rejected, approval passes only post-re-review, a post-review identity edit re-arms review, audit trail contains every round. |
...
| **AC-0-J-10** | A `retired` PIM entity can transition back to `active` via the same approval workflow. For a Product Master, the Producer activation gate is re-checked at re-activation; if the Producer is no longer `active` or its KYC is no longer cleared (now `pending` / `rejected`), re-activation is blocked. | §4.1 last paragraph + §5.4; BR-Lifecycle-2, BR-Producer-1 | AUTO — drive `retired → reviewed → active` with Producer `active` + KYC cleared (`verified` or `not_required`); then with KYC revoked to `rejected`; assert success + block respectively. |
| **AC-0-J-11** | End-to-end demo session: Paolo observes the Catalog team walking Product Master creation (manual baseline + LWIN if the adapter has landed), Product Variant creation, full chain to the Intrinsic SKU, Composite SKU creation, bulk import, rejection round, cascade retirement, Layer-1 whitelist reduction on an active Variant, re-activation. | §1–§9 (full surface) | HUMAN — single session, ~60–90 min, with dev + catalog team; Paolo signs off on observed behaviour. |
...
| **AC-0-FSM-4** | Intrinsic SKU traverses the same 4-state lifecycle with `IntrinsicSKUCreated` / `IntrinsicSKUActivated` / `IntrinsicSKURetired`. | §4.1 + §14.1 | AUTO |
| **AC-0-FSM-5** | Composite SKU traverses the same 4-state lifecycle with `CompositeSKUCreated` / `CompositeSKUActivated` / `CompositeSKURetired` — emitted as a **distinct event family** from the Intrinsic SKU. | §4.1 + §14.1 + §14.2 | AUTO |
...
| **AC-0-FSM-12** | Producer activation gate: a Product Master cannot transition to `active` unless the linked Producer (Module K) is `active` AND its KYC is **cleared** (`verified` or `not_required`) at the moment of transition; KYC `pending` or `rejected` blocks. | §5.4; BR-Producer-1 | AUTO — blocking negative paths: Producer `draft`; Producer `reviewed`; Producer `active` with KYC `pending`; Producer `active` with KYC `rejected` — each rejected. Positive paths: Producer `active` with KYC `verified`, and Producer `active` with KYC `not_required` — both admitted. |
...
## §4 Business rule enforcement (30 BRs)
...
### §4.1 Product identity (BR-Identity-1..5)

| AC ID | BR statement (verbatim from PRD §13.1) | Verification |
|---|---|---|
| **AC-0-BR-Identity-1** | Uniqueness is enforced **per Product Type on a type-defined identity key.** For `WINE` the key is **producer + product name + appellation**: no two `active` `WINE` Product Masters may share that combination. **Appellation is optional**; a **null appellation is itself a significant key value**, so two null-appellation wines with the same producer + name collide as duplicates, while an appellation-bearing wine and a null-appellation wine with the same producer + name are distinct (the key degrades to producer + name when appellation is absent). The match is **normalisation-insensitive** (case-, accent-, whitespace-folded for the comparison; stored values retain the operator's casing; algorithm is DEC-073). The persisted LWIN code is **not** part of the key. The deduplication check runs identically on both the enrichment-adapter creation path and the manual baseline path. | AUTO — positive path: create two with different identity, both succeed; negative path: attempt duplicate, assert rejection with clear reason; **appellation-null path**: two same-producer+name null-appellation entries collide, but a same-producer+name appellation-bearing entry is distinct; **normalisation path**: entries differing only in case/accents/whitespace collide; run on both the adapter and manual paths. **(Manual-path half is launch-critical; adapter-path half verified when the WINE/LWIN adapter lands.)** |
...
| **AC-0-BR-Identity-5** | A Product Master's Product Type is fixed at creation and cannot be changed for the life of the Master. A reclassification is modelled as retire + register a new Master, never a type edit. | AUTO — assert no API path mutates a Product Master's Product Type post-creation (an attempted type edit is rejected). *(At launch `WINE` is the only type, so this is a forward-seam guardrail with no reachable cross-type transition; the immutability constraint itself is testable now.)* |
...
| **AC-0-BR-Lifecycle-2** | Every PIM entity follows `draft → reviewed → active → retired`. Both **retirement (`active → retired`) and re-activation (`retired → active`)** are commercial-impact transitions that pass the **same Creator → Reviewer → Approver approval workflow** as activation (§4.2) — retiring a PIM entity requires approval. The workflow runs at the configured role-count (Q2 — §4.2), and the separation-of-duties floor (no self-approval; distinct actors on each configured step; audited) holds at any depth. | AUTO — lifecycle traversal covered by AC-0-FSM-1..7; re-activation approval by AC-0-FSM-9. **Retirement approval-gating: attempt `active → retired` with a same-actor initiator/approver, assert rejection (self-approval blocked); with distinct actors, assert the retirement succeeds and `*Retired` fires; exercise at both the 3-step and the configured 2-step (Creator → Approver) depth (cf. AC-0-GEN-12).** |
...
|---|---|---|
| **AC-0-BR-Producer-1** | A Product Master cannot transition to `active` unless the linked Producer (Module K Producer Registry) is itself `active` and KYC-cleared (`verified` or `not_required`). | AUTO — already covered by AC-0-FSM-12. |
...
### §4.5 SKU rules (BR-SKU-1..5)

| AC ID | BR statement (verbatim from PRD §13.5) | Verification |
|---|---|---|
| **AC-0-BR-SKU-1** | An Intrinsic SKU = one Product Reference + one Case Configuration + commercial attributes. The activation prerequisite is that the PR and the Case Configuration are both `active`. | AUTO — covered by AC-0-FSM-10 (iSKU → PR, iSKU → CC pairs). |
...
### §4.6 Format and Case Configuration (BR-RefData-1..3)
...
| **AC-0-BR-RefData-3** | Case Configuration has three attributes: `unitsPerCase` (integer ≥ 1); `packagingType`, a controlled, bounded base-code vocabulary stored as reference data (not free text, not a hard-coded constant); at launch a **fixed, predefined seed set** (`OWC`, `OC`, `CARTON`, `LOOSE`, each with a human-readable label that is translatable per §8 across the six launch locales — English fallback, §8.3 — and carries singular and plural forms per locale), operators selecting from it — self-serve creation of a new code (via an approved Case Configuration, normalised, with label capture) is deferred post-launch (§17.7), the set remaining extensible by config in the interim; and optional descriptive, non-identifying `physicalAttributes` (display / audit only). A Case Configuration is unique on `(packagingType, unitsPerCase)`; `physicalAttributes` are not part of the key (entries differing only in notes are the same configuration). `packagingType = LOOSE` requires `unitsPerCase = 1`; the converse does not hold (`unitsPerCase = 1` does not imply `LOOSE` — e.g. `OWC1`). The canonical display name is derived from `packagingType + unitsPerCase` (no stored name required). Physical representation is the dev team's call (DEC-073). | AUTO — assert `packagingType` accepts only a seeded code and rejects an out-of-set value; assert each seeded code carries a translatable label with singular + plural forms across the six launch locales, with English fallback when a locale is missing (§8.3), and that the derived code shorthand (`OWC6`) is not translated; create two entries with the same `(packagingType, unitsPerCase)` and assert the second is rejected as a duplicate; create two entries differing only in `physicalAttributes` and assert they collide (same config); assert `LOOSE` + `unitsPerCase ≠ 1` is rejected and `LOOSE` + `1` accepted; assert `OWC` + `1` (unitsPerCase = 1, non-`LOOSE`) is accepted; verify the display name is derived (`OWC6`, `loose`), not stored. **The self-serve new-code flow (minted-only-on-approval, normalisation / near-duplicate warning, label capture) is verified when the packaging-type creation flow lands (§17.7).** |

### §4.7 Bulk import (BR-BulkImport-1..4)

| AC ID | BR statement (verbatim from PRD §13.7) | Verification |
|---|---|---|
| **AC-0-BR-BulkImport-1** | Bulk-import depth is configurable per operation: Product Master only; + Product Variant; + Product Reference; or full chain through the Intrinsic SKU. | AUTO — drive each of the four depth modes; assert correct entity-type set produced. |
...
| AC ID | Statement | Anchor | Verification |
|---|---|---|---|
| **AC-0-EVT-14** | Module S consumes `ProductMasterActivated`, `ProductVariantActivated`, `ProductReferenceActivated`, `IntrinsicSKUActivated`, `CompositeSKUActivated` — enabling Offer creation + Price-Book entries; and the corresponding `*Retired` events to flag Offers + Price-Book entries for review. | §14.5 | AUTO — emit each event, assert Module S consumes + correct state change (verified when Module S lands). |
...
| AC ID | Statement | Anchor | Verification |
|---|---|---|---|
| **AC-0-XM-3** | PIM has no Bottle-Page-specific entity, event, or governance step. PIM publishes content at Product Master (product-level prose, incl. the **wine's** region in the `WINE` attribute set — authoritative for the wine's region on the Bottle Page), Product Variant (variant-specific prose), Product Reference (format-specific notes); Module K Producer carries producer-level description **and the Producer's own `region`** (a distinct attribute — the producer's home region, not the wine's; neither derived from the other). Module B reads these at render time; the Bottle Page render contract is verified in Module B's acceptance doc. | §9.1 + §9.2 | AUTO — inspect PIM entity schemas across all 7 types; assert no Bottle-Page entity/event/governance step on any PIM entity; assert the wine's region lives on the `WINE` attribute set (Module 0), distinct from the Producer `region` (Module K). |
...
| **AC-0-XM-9** | PIM is bottled-wine-only at launch (sole Product Type `WINE`). Liquid Product / Liquid SKU / BottlingResolution are NOT present; Service / Experience SKU subtype is NOT present; **no non-wine Product Type, attribute set, or Format is defined** (generalisation non-goal). | §1 + §3.1 + §15 + §17.1 + §17.2; DEC-065 | AUTO — schema inspection; assert PIM entity types limited to {Product Master, Product Variant, Product Reference, Format, Case Configuration, Intrinsic SKU, Composite SKU}; assert `WINE` is the only Product Type value; no Liquid / Service / Experience / non-wine types present. **See AC-0-GEN-1 / GEN-8.** |
...
| **AC-0-GEN-5** | **Identity uniqueness is a type-defined key**; `WINE` = producer + product name + appellation (appellation optional, null significant; unchanged value/behaviour for appellation-bearing wines). | PRD §13.1; brief §6 | AUTO — covered by AC-0-BR-Identity-1 (expressed as a type-defined key, incl. the appellation-null + normalisation paths). |
| **AC-0-GEN-6** | **Lifecycle events are renamed `Wine*`/`BottleReference*` → `Product*`** (`ProductMaster*`, `ProductVariant*`, `ProductReference*`) and the Intrinsic-SKU family **`SellableSKU*` → `IntrinsicSKU*`** (MVP-DEC-033); `CompositeSKU*`, `Format*`, `CaseConfiguration*`, `EnrichmentDataUpdated` unchanged; **payload semantics identical.** | PRD §14; brief §6 | AUTO — inspect the event registry; assert the renamed families are present with no `Wine*`/`BottleReference*`/`SellableSKU*` names remaining; assert payload schemas are unchanged from v1.1 semantics. (Covered for emission by AC-0-EVT-1..4.) |
