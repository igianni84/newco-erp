...
| **AC-A-BR-CrossModule-2** | Allocation creation requires `producer_id` to be `active` and KYC-verified in Module K. **(FLOOR — the upstream KYC expression; unchanged.)** | AUTO — negative paths: Producer `draft`, `reviewed`, `active`-but-not-KYC-verified, `retired`, all rejected; positive path with `active` + KYC-verified succeeds |
...
| **AC-A-EVT-19** | Module A consumes Module K `ProducerActivated` (+ KYC-verified) to enable allocation creation under that Producer per BR-A-CrossModule-2. | §11.2 + Module K §15 | AUTO — covered by AC-A-BR-CrossModule-2 negative paths + positive recovery: emit `ProducerActivated` + KYC-verified event, assert creation under that Producer now admitted |
