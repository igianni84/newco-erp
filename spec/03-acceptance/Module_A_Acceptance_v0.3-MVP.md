...
| **AC-A-EVT-25** | Module A consumes `OwnershipTransitioned` (PRODUCER → NEWCO) — audit-only delta; does NOT gate sellability. | §12.3 + §11.5.1 | AUTO — emit `OwnershipTransitioned`; assert audit recorded; assert sellability unchanged |
...
| **AC-A-XM-14** | Two-layer no-overselling guard per Q-CL-5 + DEC-185/187/196: Layer 1 (Module A allocation-pool, `qty − issued ≥ 0`, DEC-099) ∧ Layer 2 (Module B physical-inventory, `physical_in_storage − reserved − quarantined − under_adjustment ≥ 0`). Both must pass at hold placement / voucher issuance, strongly consistent; either failure rejects the hold. **Layer-2's sale-gate is sourcing-model-scoped (§7.1, MVP-DEC-027): warehouse-resident sourcing only (`passive_v2`; `direct_purchase` once received — idle at launch). `passive_v1` holds/issuances validate Layer 1 alone (no StockPosition pre-receipt; per-order arrivals are committed stock); physical receipt gates shipment, not sale (DEC-081 / Phase C item K).** **(FLOOR — unchanged; scope made explicit.)** | §7.1 + §11.5 + DEC-185 + DEC-187 + DEC-081 | AUTO — drive the four corner cases on a warehouse-resident (`passive_v2`) allocation ({L1 pass + L2 pass}→accept; the three failure combinations→reject); for each rejection assert the per-layer failure-attribution + the underlying counter values; additionally drive a `passive_v1` allocation with L1 pass and NO StockPosition: assert issuance ACCEPTED on Layer 1 alone, and the shipment subsequently held at Module C's gate until `InboundEventPhysicallyAccepted` ("in transit; ETA X") |
| **AC-A-XM-15** | Sub-pool composition at hold placement per DEC-187 + DEC-196 + BR-A-SubPool-2: SERIALIZED-offer line rejected if `atp_serialized` < requested; NON_SERIALIZED-offer line rejected if `atp_non_serialized` < requested. **Cross-sub-pool fungibility NOT admitted.** **(Within the §7.1 Layer-2 sale-gate scope — MVP-DEC-027: a `passive_v1` line composes per sub-pool against Module A's own sub-pool availability (sub-pool `qty` minus sub-pool issued vouchers, the §7 partition), not Module B ATP.)** **(FLOOR — unchanged; scope made explicit.)** | §7.1 + §11.5 + DEC-187 + DEC-196 | AUTO — drive SERIALIZED-offer with insufficient `atp_serialized` but sufficient `atp_non_serialized`, assert rejection; the inverse for NON_SERIALIZED-offer; for each assert the per-sub-pool ATP values + the partition-enforcement constraint in the rejection trace; for a `passive_v1` line assert the per-sub-pool check runs on Layer-1 sub-pool availability with no Module B read required |
