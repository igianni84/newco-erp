...
**The naming cascade (Phase C item A — the one mechanical change).** Module K applies Module 0's source-of-truth names to the **Producer → Product Master** link prose and to any prose that references Module-0 (PIM) events. **K's own event names are unchanged** — `ProducerActivated` / `ProducerRetired` / `ProducerCreated` are *K's* events that PIM **consumes**; they do not rename. Module E's category-neutral consumed-event names (`MembershipFeePaid`, `ClubCredit*`, …) are likewise unchanged (the Module E carve-out). "Wine Master" is retained as a wine-display alias for Product Master. **Naming/contract only — zero consumer behaviour change.**
...
- **AgencyAgreement entity dormant**. NewCo does not operate active consignment / third-party agency at launch; the v17 entity stays in the inherited model for future-flexibility but is not exercised by any NewCo flow (carried to roadmap, §17).
...
- **Module E** (Financial Events): records the financial events of membership-fee payment, Club Credit lifecycle, settlement, invoice issuance, and payment. Per DEC-072, Module E records the events; Xero decides GL treatment. Module E also **emits** the events Module K consumes to set Hold + Club-Credit + fee-paid state (§15.8).

---

## §2 Personas

Module K serves several operator-facing roles plus a customer-facing surface (the Consumer Portal) that reads Module K state but does not edit it, plus a producer-facing surface that at launch is read-only except for the single retained producer write.
...
- **Consumer Portal end-user (read-only).** Sees their own Customer + Profile state, their Originating Club narrative, their Club Credits, their consent state. Cannot directly edit Module K state — Customer-driven changes flow through onboarding, consent capture, and admin-mediated paths.
...
- **HubSpot integration (system actor).** Receives Module K's full customer-data sync; owns outbound marketing and transactional-email delivery; never edits Module K state directly.
...
The **Producer Portal read + full 7-section reporting (D23) is KEPT** at launch (it reads A / S / E / K). The consumer storefront / cellar / Bottle-Page reads are exempt (self-serve).
...
- **Fee-paid moment**: the timestamp at which the most recent membership fee payment was confirmed by the payment provider — set by Module K on consumption of Module E's `MembershipFeePaid` event, never by Module K independently.
...
- **Auto-renew flag**: whether the Profile auto-renews at the end of the validity period.
...
- **`Applied → Approved`** — the Producer approves the Profile (capacity allows). On the Customer's *first* approval into any Club across their lifetime, the `OriginatingClubLocked` event fires (§6). On approval, the Customer is invoiced for the Hero Package (Module S concern); fee payment via Module E moves the Profile to Active.
...
- **`Approved → Active`** — on Module E's `MembershipFeePaid` event for this Profile's first invoice (or on free-club activation where no fee applies). The Customer can now transact against the Club.
...
- **`Suspended → Active`** — when the triggering Hold is lifted.
...
- **Registration flow type**: the registration / approval flow that governs how new applications enter (open registration, application-with-approval, invitation-only, link-onboarding).
...
- **Invite-only flag**: whether the Club admits applications openly or requires Producer-issued invitations (§7).
...
- **Optional Club narrowing**: an agreement may be scoped Producer-wide or scoped to a specific Club (brand-specific sub-agreement). Both shapes are admitted; producer-by-producer call.
- **Term dates**: 24-month default per BMD §3.3; term-start and term-end record the operative window.
- **Settlement cadence**: the cadence that governs when Module E emits the per-Producer settlement event. Default is the BMD-locked quarterly cadence; per-Producer overrides are admitted (DEC-042). **This is the D19 settlement-cadence seam — recorded at launch even though the settlement engine is operator-run first (E / D19 deferred).**
...
- **`terminated`**: permanent end (early termination, dispute, mutual cancellation). Termination does *not* auto-cascade to Producer-level state changes; the Producer's own retirement is a separate operator action.
...
A **Hold** is the **unified blocking mechanism** that gates commercial activity — named un-cuttable (plan §3). A Hold attaches to one of three scopes — Customer, Account, or Profile — and one of six types — `admin`, `kyc`, `payment`, `fraud`, `compliance`, `credit`. Each Hold records the type, the reason, the actor who placed it, the moment placed, and (when lifted) the actor who lifted it and the moment lifted.
...
**Hold-lift discipline per Hold type (DEC-160).** Hold-lift discipline applies per type, not uniformly: **auto-lift permitted** on `kyc` (auto on KYC clear) and `payment` (auto on payment success); **explicit operator lift required** on `admin`, `fraud`, `compliance`, and `credit` Holds. Each lift fires the corresponding `*Reactivated` event with the actor and reason recorded.
...
1. The prospect submits email, password, name, and date of birth.
...
A Producer, marketing partner, or Operator shares a Club-specific registration link pre-bound to a specific Club:

1. The prospect submits email, password, name, date of birth — the form auto-binds the resulting Profile to the linking Club.
...
1. The Producer's invitation is recorded — **at launch the invitation is operator-driven via the Admin Panel** (the producer-facing invite UI is deferred, L-PP; the backend capability is unchanged — §3.2).
2. Module K records a `MembershipInvitationSent` event carrying the invitation identity, the inviting Club, the invitee email, the inviting actor, and the moment of sending. HubSpot consumes the event and delivers the invitation email.
...
4. On registration, Module K creates the Customer + Account + Party + Profile, fires `MembershipInvitationAccepted`, and the Profile follows the standard state machine. The invitation typically pre-approves the Profile, so it typically lands in `Approved` quickly.

#### §7.4 First approval triggers Originating Club lock

In any of the three flows, the moment a Profile transitions to `Approved` for the first time across the Customer's lifetime, the `OriginatingClubLocked` event fires (§6) and the Customer's Originating Club is set to the approving Club. This is independent of which onboarding flow the Customer came through.
...
- **HubSpot owns outbound delivery**. Invitation emails, waitlist confirmations, welcome messages, transactional emails — all delivered by HubSpot, driven by Module K events. Module K never sends a Customer communication directly.
...
- **`requested`**: the Customer opted in (a marketing-consent checkbox at registration, an opt-in form, or an admin-recorded opt-in). Module K sends a confirmation email with a confirmation link — the second leg of double opt-in (DEC-026).
...
**GDPR right-to-erasure × active Hold edge (DEC-027).** Anonymisation precedence with an active Hold is **per-Hold-type and case-by-case**: regulatory-record-retention Holds (sanctions Holds carrying OFAC / UIF retention obligations, and `compliance` Holds) **block** anonymisation until the Hold is lifted, and the Customer is informed of the regulatory delay; non-regulatory Holds (`payment`, `fraud`, `kyc`, `admin`, `credit`) **do not block** anonymisation — anonymisation proceeds and Hold metadata anonymises alongside Customer PII (the structural Hold flag is preserved as boolean for blocking-state continuity even after handle removal). The Compliance × Legal coordination playbook for the regulatory-Hold case is operating-manual scope.
...
**Reactivation paths.** A Profile or Customer transitions out of suspension when the triggering Hold is lifted. Each lift fires the corresponding `*Reactivated` event; on `Suspended → Active`, Club Credit becomes mutable again.
...
1. **Operator initiates Producer retirement.** The Producer transitions to `retired`; `ProducerRetired` fires.
...
Club Credits are auto-generated when Module K consumes Module E's `MembershipFeePaid` event and the Profile's Club has `generates_credit = true`. Issuance is **gated on payment-provider-confirmed payment success** — Module K never auto-generates a Club Credit before Module E confirms the payment intent succeeded. This pins the fee-paid → credit-issued chain to actual cash receipt. **(Core — KEPT.)**
...
- **Year-end lapse**: `valid_to` passes without full redemption. A daily background job auto-transitions credits past their `valid_to`.
...
#### §11.4 Module K records, Module E records the financial event

Module K does NOT emit Club Credit lifecycle events. **Module E emits `ClubCreditIssued`, `ClubCreditApplied`, `ClubCreditRestored`, and `ClubCreditForfeited`; Module K consumes those events and records the resulting state on its Club Credit entity.** This is the pattern across financial events (the `MembershipFeePaid` consumer pattern is the same): Module E records the financial event; Module K records the state change to its own entity. Per DEC-072, Xero decides GL treatment. *(Phase C item D: the three-way seam **entity (K) ↔ events (E) ↔ redemption (S)** holds; the deferred K.18/K.19 paths simply don't fire, so Module E records nothing for them — no Module-E cut.)*
...
- **Right of access / data portability.** A Customer can request export of their personal data and transactional history in a standard format. Operationally executed by Compliance + Customer Care; not modelled as a state machine.
...
For any given Club: the count of currently `Active` Profiles ≤ the current quantity of that Club's Hero Package allocation. The cap is evaluated at every Profile transition into `Active` (membership approval; renewal continuing into a new period).

`Lapsed` and `Cancelled` Profiles do not consume capacity — only `Active` Profiles do. A lapsing Profile frees capacity; a re-activation within the 30-day grace re-consumes capacity (subject to the cap at re-activation time).
...
A Hero Package is a Module S Offer-level **designation** — a role attached to an Offer that backs the Club's annual membership purchase — not a structural type at PIM. It can be backed by any standard PIM artefact: a single-PR Sellable SKU (Intrinsic), a Composite SKU, or any future SKU shape. The capacity arithmetic binds to the underlying Allocation's quantity regardless of which SKU shape backs the Hero Package Offer.
...
- **Decrease constraint**: a Producer cannot reduce the quantity below the current count of `Active` Profiles (would orphan members). Reductions above the current count are admitted but rare. Tighter constraints are future-flexibility (§17).
...
**BR-K-Identity-1. Unique email.** Email is globally unique across all Customers at NewCo. Email changes follow a verification + old-email confirmation workflow.
...
**BR-K-Customer-3. Customer with zero active Profiles is flagged for review.** The Customer is not auto-suspended; the flag enables ops follow-up. Auto-suspension on zero active Profiles is a future-flexibility item.
...
**BR-K-Agreement-2. Settlement-cadence override.** The active ProducerAgreement's settlement cadence governs the per-Producer settlement event timing in Module E. Default cadence is the BMD-locked quarterly; per-Producer overrides are admitted via the agreement. *(D19 settlement engine deferred — operator-run first cycles; the recorded cadence is the seam.)*
...
**BR-K-Contract-3. HubSpot owns outbound communication.** HubSpot reads Module K's full customer-data sync and delivers all outbound customer communications. Module K never sends a Customer communication directly.
...
- `ProfileActivated` — on `Approved → Active` (membership-fee paid; or free-club activation). Module K consumes Module E's `MembershipFeePaid` to drive this.
...
- **`MembershipInvitationSent`** (§7.3). Fires when a prospect is invited to a Club (operator-driven at launch — §3.2). Consumers: HubSpot (delivers the invitation email); audit.
- **`MembershipInvitationAccepted`** (§7.3). Fires when an invitee accepts. Consumers: Module K's Profile-creation flow; HubSpot.
- **`WaitingListJoined`** (§4.2.1). Fires when a Profile transitions to `WaitingList`. Consumers: HubSpot (waitlist-confirmation); the capacity-invariant evaluator (§13).
...
Module K does NOT emit Club Credit lifecycle events or the membership-fee-paid signal. Module E emits these; Module K consumes them and records the resulting state on its own Profile / Club Credit / Hold entities. **(Module E's names are category-neutral and unchanged — the Module E carve-out, §18.)**

- `MembershipFeePaid` — emitted by Module E on payment-provider-confirmed payment success for an INV0 charge. Drives `Profile.fee_paid_at`, `ProfileActivated` / `ProfileRenewed`, and Club Credit auto-generation (§11.1).
- `ClubCreditIssued` / `ClubCreditApplied` / `ClubCreditRestored` / `ClubCreditForfeited` — emitted by Module E. Module K records the Club Credit lifecycle state on its own entity. *(The K.18 welcome-window-scaled-issuance path and the K.19 manual-issuance path are deferred at launch, so the corresponding issuance variants simply don't fire — §11.1.)*
...
- **Allocation creation, capacity management, sub-pool carve-out, sourcing-model attribute, Hero Package allocation `qty` storage.** Module A. Module K enforces the Hero Package capacity invariant against Module A's stored `qty`; it does not duplicate the value.
- **Product Master / Product Variant / Product Reference / Sellable SKU / Composite SKU** *(wine-display aliases: Wine Master / Wine Variant / Bottle Reference)*. Module 0 (PIM). Module K holds the Producer entity that PIM links to; it does not duplicate Module 0 state. *(Naming cascade — §18.)*
...
- **AgencyAgreement / third-party-owner consignment / active consignment.** Inherited entity present for future-flexibility but **not active at launch** (DEC-011 + DEC-017). The `AgencyAgreement*` / `AgencyConsentGranted` event family is dormant (carried to roadmap — §17).
...
- **HubSpot communication delivery.** HubSpot owns outbound customer communication. Module K emits events; HubSpot reads via the full customer-data sync and delivers messages. Module K never sends a Customer communication directly.
...
| §16 boundary list | "**Wine Master / Wine Variant / Bottle Reference** / Sellable SKU / Composite SKU" | "**Product Master / Product Variant / Product Reference** / Sellable SKU / Composite SKU" | Wine Master / Wine Variant / Bottle Reference |
...
- **Module E's consumed-event names are unchanged** (the Module E carve-out — category-neutral): `MembershipFeePaid`, `ClubCredit{Issued,Applied,Restored,Forfeited}`, `CustomerCreditHold{Placed,Lifted}`, `CustomerChargebackFlagged`, `StoragePayment{Failed,Succeeded}`.
