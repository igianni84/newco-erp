...
6. **Module E owns RECONCILE R4 — and ⚠️ lands E-EMITS `SupplierPaymentCompleted` (the trap; the cut-sheet says "D-emits"; Phase C flips it).** The cut-sheet's RECONCILE (E.32 / §3.6 / Q6) framed `SupplierPaymentCompleted` as "**Module D** emits; E + B consume." **Phase C ratification (Paolo Q2) FLIPPED this to E-emits — and Phase C WINS.** **Module E emits `SupplierPaymentCompleted`** on payment clearing (E is the payment executor — three-actor split DEC-119; symmetric with the customer-side `AirwallexChargeExecuted`); **Module D consumes it** → settle/close the PO; **Module B consumes it** → inventory `ownership_flag` PRODUCER→CRURATED. At launch E emits it when the operator records the manual supplier payment in E's finance surface (settlement operator-run, D19 deferred); post-launch via E's settlement engine. Atomic per PO (partial PO settlement deferred, OQ-20). This PRD **moves `SupplierPaymentCompleted` from Module E's *consumed-from-D* set into its *emitted* set** (§8.1 / §8.2 / §9.2 / §9.6 / §5.9 / AC-E-J-37). **N3:** the two ledgers are distinct — the inventory `ownership_flag` `CRURATED` (B, keyed to `SupplierPaymentCompleted`) vs the PO-level title `NEWCO` (D, keyed to the sale signal `VoucherIssued`) — same real-world party, two ledgers, two signals (§5.9). **Item F:** name `VoucherIssued` (sell-through) + `VoucherShipped` (shipment leg); **take no accounting position** (DEC-072), folded into the deferred settlement (§5.9). (Ratified Q6; Phase C R4/N3/item F.)
...
> **One drift flagged for Paolo (not a cut; a forward-consistency naming reconciliation).** The settled Module K sibling consumes its Club-Credit lifecycle events from Module E as `ClubCreditIssued` / `ClubCreditApplied` / `ClubCreditRestored` / `ClubCreditForfeited` (Module K §15.8). v1.1 Module E named the issuance event `ClubCreditAccrued` (+ `StoreCreditIssued` / `StoreCreditApplied` / `ClubCreditApplied`). This PRD **aligns E's emission names to Module K's drafted registry** (`ClubCreditIssued`/`Applied`/`Restored`/`Forfeited` + the `StoreCredit*` analogs) so the K↔E credit-event contract reads consistently — naming/contract only, no behaviour change; Module E's `ClubCredit*`/`StoreCredit*` family stays category-neutral. Flagged so Paolo can confirm the reconciliation (digest). (§5.3, §12.)
...
- **Stage 1 — retry within the 14-day window.** *(At launch:)* the operator monitors the failed INV3 charge in the Admin Panel; the saved-card re-charge + the HubSpot payment-reminder are operator-triggered. *(The DEC-160 Airwallex built-in retry + the auto-`AirwallexChargeFailed`-per-attempt audit + the auto-reminder are the deferred automation; the retry counter + the per-invoice threshold are retained.)* No Hold; no Suspension.
...
Hero Package = membership purchase. Fires INV1 like any other purchase (no separate INV0). Module E charges (card §3.1 / bank-transfer §3.2) + routes to Xero; Module S emits `MembershipFeePaid` (→ Module K auto-issues Club Credit if `Club.generates_credit = true`); **Module E records the `MembershipFeePaid` financial event.** No separate membership-billing flow; renewal follows the same pattern (Module-K + Module-S scope).
...
Module S emits the refund event; Module E executes the Airwallex refund. The five-step flow: (1) Module S emits `RefundRequested` (Voucher + Order ref + `refund_cause` + amount + currency); (2) Module E reads the original Payment record + calls the Airwallex refund API + emits `RefundExecuted`; (3) Module E confirms refund-completed to Module S (Order state + HubSpot customer-notification trigger); (4) Module E routes `RefundExecuted` to Xero (§7.1); (5) the producer-fault clawback aggregation defers with the engine (§4.5).
...
**Module E records** (aligned to Module K's drafted registry — §15.8 — for forward-consistency; the digest flag): `ClubCreditIssued` / `ClubCreditApplied` / `ClubCreditRestored` / `ClubCreditForfeited` + the `StoreCreditIssued` / `StoreCreditApplied` analogs. Module K consumes these to record the balance state on its own entity. **Module E does NOT own the credit-balance entity** — it reads via the Module K API for event-payload context (issuer, applied-amount, remaining-balance).
...
- **Module B consumes it** (independently) → the inventory **`ownership_flag` PRODUCER → CRURATED** transition (Module B §0.3/§2.2 — the bottle becomes NewCo-owned because NewCo has paid for it). **Module B does NOT emit it; Module E does.** No fresh Module E recording fires on B's downstream `OwnershipTransitioned` (Module E observes it for audit only — the financial event is the E-emitted `SupplierPaymentCompleted`).
- **Direct-Purchase no-op:** for `direct_purchase` the InboundBatch is `CRURATED` from creation → no PRODUCER→CRURATED transition (Module E's financial event still fires for the cost-out-the-door recording). Doubly moot at launch (Direct Purchase deferred, item I).
...
|---|---|---|---|
| **Inventory `ownership_flag`** | **Module B** (consumer of the E-emitted event) | `ownership_flag` `PRODUCER → CRURATED` (DEC-185 — `CRURATED`) | **`SupplierPaymentCompleted`** (E-emitted; the **payment** moment — R4) |
...
`CRURATED` (inventory flag, B) and `NEWCO` (PO-level title, D) **denote the same party** (pre-existing v1.1 naming — DEC-085 renamed the PO enum's middle value, Module B's inventory flag retained `CRURATED` per DEC-185).
...
- **`SupplierPaymentCompleted`** — **emitted by Module E on supplier-payment clearing** (E is the payment executor; at launch via the operator's manual settlement record in E's finance surface [D19 deferred]; post-launch via the engine). **Consumed by Module D** (settle/close the PO) **and Module B** (inventory `ownership_flag` PRODUCER→CRURATED) — independently. Atomic per PO (partial PO settlement deferred, OQ-20). *(Phase C R4; supersedes the cut-sheet/v1.1 "D-emits." §5.9.)*
...
**Credit lifecycle (the financial-event side; the entity at Module K — aligned to K §15.8):** `ClubCreditIssued` · `ClubCreditApplied` · `ClubCreditRestored` · `ClubCreditForfeited` · `StoreCreditIssued` · `StoreCreditApplied`.

### §8.2 Module E CONSUMES at launch

**From Module S:** `InvoiceINV1Issued` / `InvoiceINV2Issued` / `InvoiceINV3Issued` (route to Xero + charge) · `RefundRequested` (execute Airwallex refund) · `OrderPaymentPending` (watch for `transfer.received`) · `StoreCreditApplied` / `ClubCreditApplied` (record) · `MembershipFeePaid` (record) · `StorageFeeAccrued` (informational) · `StorageFeeProRataRefundIssued` (record, cause-conditional) · `DiscoveryRevenueShareAccrued` (record as `OCShareAccrued` — the OC seam) · `DiscoveryRevenueShareReversed` (record the reversal) · `SupervisorOverridePostDeliveryRefund` (execute refund). *(Module S emits all three customer-facing invoices + the OC accrual + the refund family per DEC-119 / ratified S §16/§17.7.)*
...
- **HubSpot (DEC-014).** Module E emits trigger events (`StoragePaymentFailed`, `CustomerChargebackFlagged`, `RefundExecuted`); HubSpot decides recipient + channel + template.
...
- **Module E emits `SupplierPaymentCompleted`** (§5.9, R4); **Module B consumes it** → the inventory `ownership_flag` PRODUCER→CRURATED transition (Module B §0.3/§2.2 — **B does NOT emit it; E does**). The contract is event-driven (no synchronous coupling); the Direct-Purchase no-op (InboundBatch `CRURATED` from creation) is moot at launch.
