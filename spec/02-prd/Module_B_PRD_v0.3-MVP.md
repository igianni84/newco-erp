...
- **Date**: 2026-06-08
- **Status**: **DRAFT — awaiting batch ratification (Paolo).** Held in `mvp/`; **nothing is promoted to `handoff/` until Phase E** (the single coherent handoff). Module B is **KEPT WHOLE on its inventory-integrity floor** (the two-layer no-overselling guard Layer 2, committed-inventory protection, InboundBatch, StockPosition, the four-way reconciliation discipline), with the **two heaviest critical-path levers of the whole exercise landing in-module: D12 (DECOUPLE the NFT/on-chain layer off the launch critical path — the per-bottle serialization workflow stays launch-ready) + D16 (SIMPLIFY the Stage-8 workflow *automation* → manual-first, integrity core KEPT).** Module B is the **consumer side of RECONCILE R4** (`SupplierPaymentCompleted` is **E-emitted**, B-consumed for the `ownership_flag` PRODUCER→NEWCO flip — the cut-sheet's "D-emits" framing is superseded) and lands editorial notes **N1** (D16 manual-first, identical with Module D) + **N3** (the two-ledger clarity — the inventory-flag vs the PO-title, both `NEWCO`).
...
- **Predecessors / inputs** (the canonical record governs where this PRD is terse):
  ...
  - [`../01-triage/Phase_C_Reconciliation_v0.1.md`](../01-triage/Phase_C_Reconciliation_v0.1.md) — the **coherence gate** (RATIFIED 2026-06-07). **R4** (E-emits `SupplierPaymentCompleted` — Module B is a consumer side, §2-C/§5-R4) + **N1** (D16 manual-first — land identically with Module D, item H) + **N3** (party naming — the inventory-flag vs the PO-title, both `NEWCO`) + **item G** (the two-layer no-oversell build-sequencing — floor) + **item H** (D16 depth — B decided manual-first in lockstep; discharges D's KEEP-pending-B-review) + **item I** (Direct Purchase deferred) + **item J** (the NFT/on-chain DECOUPLE — graceful degradation everywhere; the NS path is the universal fallback) + §6 floor chains (no-overselling · committed-inventory · audit/retention).
...
3. **Module B is the consumer side of RECONCILE R4 — the E-emits trap (naming/contract only — zero behaviour change).** The cut-sheets (D.24/E.32) read "Module D emits `SupplierPaymentCompleted`." **Phase C ratification (Paolo Q2) flipped this to E-emits:** **Module E emits `SupplierPaymentCompleted`** on payment clearing (the payment executor — three-actor split DEC-119; symmetric with the customer-side `AirwallexChargeExecuted`); **Module B consumes it** to flip the inventory `ownership_flag` PRODUCER → NEWCO (B.2). This PRD **reconciles B.2's loose "V1/V2 reach NEWCO at sell-through" prose to "on `SupplierPaymentCompleted`"** and keeps the **inventory-ownership ledger (B, keyed to `SupplierPaymentCompleted`) distinct from Module D's PO-level title ledger (D, keyed to `VoucherIssued`) (N3)** — same party, both `NEWCO`, two ledgers, two signals (§0.3).
...
| **RECONCILE R4 (consumer side — E-emits, naming/contract only)** | `SupplierPaymentCompleted` is **E-emitted / B-consumed** → `ownership_flag` PRODUCER→NEWCO; reconcile §2.2's sell-through prose; N3 two-ledger clarity (inventory-flag vs PO-title, both `NEWCO`). | §0.3, §2.2, §19.2 |
...
### §0.3 R4 + N3 — `ownership_flag` PRODUCER→NEWCO on the E-emitted `SupplierPaymentCompleted` (the E-emits trap; Phase C §2-C/§5-R4)

This is the **single highest-risk reconciliation this PRD lands** — and it corrects a trap. The cut-sheets (D.24/E.32) and the v1.1 PRD §2.2 carry framings reading "Module D emits `SupplierPaymentCompleted`" (v1.1 §2.2 cites "Module D §12") and "V1/V2 reach NEWCO at sell-through." **Phase C ratification (Paolo Q2) flipped this to E-emits:** Module D has no independent trigger — it would wait on Module E's confirmation that the payment cleared. **Payment execution is Module E's** (the Airwallex/Xero rails, DEC-014/028; the three-actor split DEC-119 assigns PAYMENT to E; symmetric with the customer-side `AirwallexChargeExecuted`, which E emits).
...
- **Module B consumes it** → the inventory `ownership_flag` transition **PRODUCER → NEWCO** (§2.2; the bottle becomes NewCo-owned because NewCo has paid for it). The consumer behaviour is unchanged — **only the source is corrected (from E, not D)** and the trigger is named (the payment moment, not "sell-through").
- **Module D also consumes it (independently)** → settle/close the PO (the PO-level title ledger; Module D §6/§14.5).
- **Direct-Purchase no-op:** for `direct_purchase` the InboundBatch is `NEWCO` from creation → no PRODUCER→NEWCO transition. Doubly moot at launch (Direct Purchase deferred, §0.4).

**Reconcile B.2's prose (landed in §2.2):** the loose "V1/V2 reach `NEWCO` at sell-through" is reconciled to "**on `SupplierPaymentCompleted`**" — the inventory `ownership_flag` ledger keys to **payment**, distinct from the PO-level title ledger, which keys to the **sale/shipment signal** (`VoucherIssued`).
...
> |---|---|---|---|
> | **Inventory `ownership_flag`** | **Module B** (this PRD, §2.2) | `ownership_flag` `PRODUCER → NEWCO` (DEC-185; `NEWCO` per MVP-DEC-028) | **`SupplierPaymentCompleted`** (E-emitted; the payment moment — R4) |
...
>
> The inventory `ownership_flag` (Module B) and the PO-level title (Module D) **denote the same real-world party** and now share the `NEWCO` label (DEC-185 for the inventory flag, DEC-085 for the PO enum; the inventory flag was harmonized to the shared `NEWCO` label by MVP-DEC-028). The `OwnershipTransitioned` cascade prose in this PRD (§2.2, §19) is unambiguous about which signal each ledger keys to — so the inventory-ownership ledger (B, keyed to `SupplierPaymentCompleted`) and the title ledger (D, keyed to `VoucherIssued`) are never conflated.
...
### §0.4 Item I — Direct Purchase `→NEWCO`-at-issuance not-exercised-at-launch (Phase C item I)

Direct Purchase is **deferred at launch** (confirmed at ratification — no launch deal; passive V1 + V2 only). Module B's `direct_purchase → NEWCO`-at-issuance ownership derivation (§2.2 — the InboundBatch is `NEWCO` from creation for `direct_purchase`) is therefore **not-exercised-at-launch** — a **scope annotation, not a cut** (consistent with Module A keeping the `direct_purchase` enum + Module D deferring the use; all five of A/D/B/E/S idle the path in lockstep). The `sourcing_model` discriminator + the at-issuance derivation are **retained as the seam**; re-enable is additive. At launch V1/V2 stock reaches `NEWCO` via the E-emitted `SupplierPaymentCompleted` (§0.3) — the Direct-Purchase no-op is doubly moot.
...
- **StockPosition aggregated view** (§8) — 5-dimension aggregation `(PR, warehouse, case_config, allocation, ownership)`; sellable + shippable; sub-pool decomposition feeding Module A + Module S + Module C; + the **`received_to_date` per-allocation receipt-position read** (§8.3 — MVP-DEC-037) feeding the Admin-Panel allocation-position view + the D23 producer-reporting read.
...
|---|---|---|
| **Ownership** | Legal ownership | 2-value enum: `PRODUCER`, `NEWCO` (§2.2; v17's `THIRD_PARTY` value is OUT — already-deferred, §21) |
...
The **StockPosition aggregated view** (§8) reports total / committed / available quantities at the canonical 5-dimension intersection `(PR, warehouse, case_config, allocation, ownership)`. **Sellable quantity** feeds Module A + the storefront (commercially `available`, in storage, not quarantined); ownership flag is independent of commercial availability (`PRODUCER`-owned V2 consignment stock and `NEWCO`-owned stock are both sellable). **Shippable quantity** feeds Module C (committed, in storage or reserved for picking, not quarantined, not under adjustment).

### §2.2 `ownership_flag` at NewCo — 2-value enum + the R4 E-emits transition **(R4 / N3 landed — §0.3)**

The `ownership_flag` enum at NewCo launch is **2-value**: `PRODUCER` and `NEWCO` (the v17 third value `THIRD_PARTY` is OUT — already-deferred, §21).
...
`StockPosition = (Product Reference, Warehouse, Case Configuration, Allocation, Ownership)` *(naming cascade: `Bottle Reference → Product Reference`; the physical-unit semantics unchanged)*. Each dimension is load-bearing: **Product Reference** (catalog-level identity); **Warehouse** (physical custody at warehouse granularity; multi-warehouse already-deferred); **Case Configuration** (load-bearing for case-integrity-aware ATP — **cannot** be dropped, or mixed-case unbreakable dispatch breaks; **populated at receiving from the Logilize-reported packaging — BR-B-Reconcile-5**); **Allocation** (immutable UUID; never interchangeable across allocations); **Ownership** (2-value enum, §2.2). Three headline quantities at the intersection: `total_quantity` (serialized SerializedBottle count + NS InboundBatch residual); `committed_quantity`; `available_quantity` (`total − committed − reserved − quarantined − under_adjustment`).

### §8.2 Sellable vs shippable + sub-pool decomposition

**Sellable** feeds Module A + the storefront (commercially available, in storage, not quarantined; ownership-independent). **Shippable** feeds Module C (committed, in storage or reserved, not quarantined/under-adjustment); the case-config dimension is critical — when `effective_unbreakable = true`, Module C's late-binding selection reads shippable quantity at the case-config level (whole-case dispatch). **Sub-pool decomposition:** StockPosition exposes `available_serialized` (count of SerializedBottle records in `available` status) + `available_non_serialized` (the NS-pool ATP, §5.2) at the allocation dimension; both feed Module A's two-layer guard (§10.5). **Module C reads StockPosition at late-binding pick** (the named B↔C contract — §0.6). *(Sale-side note — MVP-DEC-027: a `passive_v1` allocation has no StockPosition until its per-order receipt; its sale-side availability is Module A Layer 1 alone (§10.5 sale-gate scope). Arrived `passive_v1` units enter StockPosition as committed stock backing their vouchers — the shippable-side surfaces are unaffected.)*

### §8.3 Received-to-date per allocation — the receipt-position read **(MVP-DEC-037)**

Module B exposes **`received_to_date` per allocation** — the cumulative physically-received quantity: **Σ across the allocation's InboundBatches of `received_quantity`** (§3.1 — the Logilize-reported physical-match figure, DEC-194) — as a first-class read at the Allocation dimension, alongside (not inside) the §8.1 current-stock quantities. It is **cumulative intake, not current stock**: shipment, adjustment, quarantine, and destruction never decrement it (those move the §8.1 quantities); a DiscrepancyResolution qty adjustment flows through the batch per §3.4 / Module D §13.3. **This physical-match figure is the canonical "received" in every operator- and producer-facing report** (BR-B-Ledger-6) — not Module D's documents-side accepted qty (the `InboundEventPhysicallyAccepted` payload), which can diverge from the physical count until the variance resolves (§3.4, §11).

**Consumers**: the **Admin-Panel allocation-position view** (Admin Panel PRD §3.A — the four-figure strip: committed `qty` [Module A §4.1] / `received_to_date` [this read] / issued [Module A's Layer-1 counter] / available-to-sell [the Module S §8.6 lesser-of read]) and the **D23 producer-reporting read** (Admin Panel PRD §2.2 — the sell-through definition anchors on this figure). **Module A is NOT a consumer** — the Allocation record stays commitment-only (`qty`, DEC-079); the committed-vs-received comparison lives at the read/report layer, never as Module A or cross-module state. **No new event, no new entity** — the read derives from the InboundBatch ledger; whether it is materialised or computed on read is tech-implementation (DEC-073).
...
ATP is recalculated and the delta pushed at every §10.2 event. **PRD-level latency contract:** display ATP staleness ≤5s (Module S's storefront read tolerates it); transactional ATP real-time strongly consistent (hold placement). The literal SLA + retry + alarming are tech (DEC-073). **Storefront read path (FLOOR — Module S §8.6):** Module S reads the **lesser of** (Module A allocation-pool ATP) and (Module B physical-inventory ATP) per sub-pool — `min(Layer 1, Layer 2)` — exposing the minimum as the available-to-sell quantity. **Sourcing-model scope (MVP-DEC-027):** the lesser-of read applies to warehouse-resident sourcing (`passive_v2`; received `direct_purchase`). For **`passive_v1`** Offers Module S reads **Module A Layer-1 per-sub-pool ATP alone** (Module S §8.6): a `passive_v1` allocation has no StockPosition pre-receipt and its per-order arrivals are committed stock — the absent/zero Layer-2 read is by-design and MUST NOT zero the storefront or block hold placement. **Module B must keep exposing per-sub-pool ATP** for Module S's lesser-of read (a Module S ratified contract — §0.6 / Q4).

### §10.5 Two-layer no-overselling guard **(FLOOR — the inventory-integrity half of the scope floor)**

Both layers must pass at hold placement / voucher issuance: **Layer 1 — Module A allocation-pool layer** (`qty − issued ≥ 0`); **Layer 2 — Module B physical-inventory layer** (`physical_in_storage − reserved − quarantined − under_adjustment ≥ 0`). Either failure rejects the hold; both are evaluated at the transactional boundary. **Layer-2 sale-gate scope (sourcing-model-scoped — MVP-DEC-027; mirrors Module A §7.1):** Layer 2 gates the **sale** only where the allocation's sellable stock is **warehouse-resident** — `passive_v2` always; `direct_purchase` once physically received (pre-receipt Direct-Purchase sells on Layer 1 alone, DEC-081; idle at launch). For **`passive_v1`** (per-order inbound — BMD §3.7/§5.1) the sale is guarded by **Layer 1 alone, always**: no StockPosition exists pre-receipt (§3.1 / DEC-195) and per-order arrivals are committed to already-issued vouchers, so Layer-2 ATP never constrains a `passive_v1` hold; physical receipt gates shipment (Module C; Phase C item K), never the sale. Arrived `passive_v1` units remain fully guarded — committed-inventory protection (§13.4/§13.5) + Module C's no-oversell-at-pick read (§8.2). The two layers compose **per sub-pool**: a SERIALIZED-offer voucher line is rejected if `atp_serialized` < requested qty; a NON_SERIALIZED-offer voucher line is rejected if `atp_non_serialized` < requested qty; **cross-sub-pool fungibility is NOT admitted at hold placement** (the partition is enforced strictly). Per-sub-pool composition means the guard is **intact even when the serialized sub-pool is dormant** (the D12 decouple — at launch, if the on-chain workstream is slipping, Layer 2 = the NS sub-pool ATP; §0.5). **Composition note (the `AllocationCapacityExhausted` drift — §0.5):** Module A v0.3-MVP frames Layer-1 over-issuance as an **operation-level rejection — there is NO `AllocationCapacityExhausted` event**; Module B's Layer-2 / no-oversell prose composes with the rejection at the issuance operation, not a non-existent event (resolved consistently with Module S, which flagged the drift). The guard composes with **Module S's lesser-of read (§8.6) ∧ Module C's no-oversell-at-pick StockPosition read (§8)** — the floor whole end-to-end per sub-pool (Phase C item G/M).
...
The receiving discrepancy authority is **split** (DEC-194): **Module D = documents in order** (the 3-gate inbound QC at `PHYSICALLY_ACCEPTED` — paperwork [quantity vs the producer's manifest — the PO for direct purchase], provenance [Product Reference identity match], physical-condition-on-arrival; Module D PRD §7 / MVP-DEC-032; pass fires `InboundEventPhysicallyAccepted`, which creates the InboundBatch); **Module B = physical match** (compares Logilize-reported physical counts against the InboundBatch's expected quantity; variance triggers `InboundBatchDiscrepancy`). The independent physical-match check is the integrity primitive that detects Logilize-side errors the single-stage Module D check cannot — it delivers the KPI "inbound physical-discrepancy rate <5%" (§22.1). **This is the integrity core — not "sophistication" — KEPT.**
...
- **BR-B-Ledger-5** — ownership-flag transition discipline **(R4 landed, §0.3):** Module B records and executes the `PRODUCER → NEWCO` transition but does not decide it — Module D sets the initial flag at InboundBatch creation; **Module E emits `SupplierPaymentCompleted`** (the payment executor — B consumes, does NOT emit, the E-emits trap); Module B preserves custody history + allocation lineage across the transition. *(N3: the inventory `ownership_flag` ledger [Module B] keys to `SupplierPaymentCompleted`; distinct from Module D's PO-level title ledger [Module D], keyed to `VoucherIssued` — same party, both `NEWCO`, two signals.)*
- **BR-B-Ledger-6** — receipt-position read *(MVP-DEC-037)*: `received_to_date` per allocation = Σ of the allocation's InboundBatch `received_quantity` (the DEC-194 physical-match figure — **the canonical "received"**; Module D's documents-side accepted qty is never the reporting figure). **Cumulative intake** — never decremented by shipment / adjustment / quarantine / destruction; reflects DiscrepancyResolution qty adjustments (§3.4). Exposed at the Allocation dimension (§8.3) for the Admin-Panel allocation-position view + the D23 producer-reporting read; **Module A is not a consumer** (the Allocation stays commitment-only).

### §18.3 No-overselling guards **(FLOOR)**
- **BR-B-NoOversell-1** — two-layer guard (Module A Layer 1 `qty − issued ≥ 0` ∧ Module B Layer 2 `physical_in_storage − reserved − quarantined − under_adjustment ≥ 0`; either failure rejects the hold). **Layer-2's sale-gate is sourcing-model-scoped (§10.5, MVP-DEC-027): warehouse-resident sourcing only (`passive_v2`; received `direct_purchase`) — `passive_v1` holds are guarded by Layer 1 alone, physical receipt gating shipment not sale (DEC-081; Phase C item K).**
...
- **BR-B-Reconcile-5** — **case-configuration capture at receiving** *(launch = capture; expected-vs-actual verification = post-launch seam — MVP-DEC-030)*. At the physical-match check (§11.1), Module B records the **received Case Configuration** (packaging type + units-per-case — a reference to the Module 0 Case Configuration entity, §3.6) onto the Case / StockPosition case-config dimension (§8.1) from the Logilize-reported receipt, so the 5-dimension StockPosition is populated from *actual* receipt, not assumed — the whole-original-wooden-case provenance/value stays on the ledger. **Launch = capture, manual-first** (the operator records the packaging as received via the Admin Panel + Logilize; the storage representation is dev's, DEC-073). **Seam (P1):** automated *expected-vs-actual* verification — comparing the received Case Configuration against a derivable expected packaging (e.g. the ordering Offer's Sellable-SKU Case Configuration, or a producer's-manifest-declared packaging) and raising `InboundBatchDiscrepancy` with a **new `case_config_mismatch` variance type** — is **post-launch** (the `InboundBatchDiscrepancy` event + the DISCREPANCY state are the retained seam; **no `case_config_mismatch` variance is emitted at launch**). No data debt — the received-packaging record is exactly what the deferred verification will consume.
...
**Inventory-ledger events (FLOOR — KEPT):** `BottleStateChanged`, `OwnershipTransitioned` *(`PRODUCER → NEWCO`; triggered by the **E-emitted** `SupplierPaymentCompleted` — R4; recorded by Module B for provenance)*, `InboundBatchCreated`, `InboundBatchStateChanged`, `InboundBatchDiscrepancy` *(consumed by Module D; the automated round-trip manual-first — N1)*, `BatchSerializationDiscrepancy`, `InventoryAdjusted` *(parameterised `adjustment_type`; consumed by Module A [ATP push] + Module E [financial event])*, `InventoryShortfallDetected` *(the committed-inventory short-circuit; consumed by Module A)*, `StocktakeReconciled`, `StocktakePlanned`/`StocktakeStarted`, `BottleQuarantined`, `BottleQuarantineResolved`, `CaseIntegrityChanged`. **The NS informational event** `BottleShippedAsNonSerialized` *(the D12 universal fallback — §5.6)*.
...
- **Module S** — Module B exposes the per-sub-pool Layer-2 ATP for S's lesser-of storefront read (§10.4; lesser-of within the §10.5 sourcing-model sale-gate scope — `passive_v1` Offers read Layer 1 alone, MVP-DEC-027); consumes `VoucherIssued` (→ serialization) + `VoucherShipped` (→ NFT burn — **decoupled, D12**; NS fires `BottleShippedAsNonSerialized`). The Bottle Page is B's (DEC-024); S reads B's Bottle Page link. **(Module S's four ratified contracts honoured whole — Q4.)**
...
| §0.3 R4 + N3 | — | Phase C §2-C/§5-R4/§5-N3 | NEW — the E-emits `SupplierPaymentCompleted` consumer side; B.2 sell-through prose reconciled; the two-ledger clarity (inventory-flag vs PO-title, both `NEWCO`). |
