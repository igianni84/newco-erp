...
| **Case-config** | **Inbound case-configuration verification** (Module B, §11.1 / BR-B-Reconcile-5 — MVP-DEC-030) | The **automated expected-vs-actual** check — comparing the received Case Configuration against a derivable expected packaging (the ordering Offer's Sellable-SKU Case Configuration, or a producer's-manifest-declared packaging) + a **new `case_config_mismatch` `InboundBatchDiscrepancy` variance type** → **capture-only at launch** | **Capture is whole at launch (BR-B-Reconcile-5):** Module B records the received Case Configuration onto the StockPosition case-config dimension (§8.1) at the physical-match check — no data debt (the record is exactly what the deferred verification consumes). The `InboundBatchDiscrepancy` event + the DISCREPANCY state + Module D's 6-path resolution enum are the retained seam; no `case_config_mismatch` variance is emitted at launch. | Add the expected-vs-actual comparison + the `case_config_mismatch` variance on the kept capture + discrepancy machinery. Additive. | First received-packaging mismatch incident / post-launch receiving-integrity telemetry (or when a derivable "expected packaging" source is formalised on the producer's manifest). |
...
| **Module B** | The `direct_purchase → NEWCO`-at-issuance ownership derivation is not-exercised (V1/V2 reach `NEWCO` via `SupplierPaymentCompleted`, R4) | The derivation is a **scope annotation, not a cut** — InboundBatch would be `NEWCO` from creation for `direct_purchase` (no PRODUCER→NEWCO transition). |
...
- **Sale-gate at restore (MVP-DEC-027)**: restored Direct-Purchase allocations inherit the **sourcing-model-scoped Layer-2 sale-gate** — pre-receipt in-transit sales proceed on **Layer 1 alone** (DEC-081; the voucher displays "in transit; ETA X"), Layer 2 gating the sale only once the stock is warehouse-resident (received). The same carve-out `passive_v1` uses at launch (Module A §7.1 / Module B §10.5 / Module S §8.6) — re-enable stays additive; no guard rework.
...
| **Third-party-owner `THIRD_PARTY` ownership value + AgencyAgreement entity** | Out at launch (2-value `ownership_flag` PRODUCER/NEWCO; no agency intake, no third-party custody — Module B §21; Module K §17.2 #9). | Active-consignment / agency reintroduction |
...
| **Transactional-email-provider operational choice** (EXT-4) | The ERP owns operational sends through the single email service (Module K §14.9.1 — MVP-DEC-035; supersedes the "HubSpot named for delivery" premise); the provider behind the adapter (dedicated ESP vs HubSpot transactional API) + template-engine + bounce-handling mechanics are downstream tech (DEC-073); sending subdomains split transactional vs marketing. | Downstream tech selection |
