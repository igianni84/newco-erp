...
- **Active: RM-01 🟡 — change `parties-anonymisation` AUTHORED + `validate --strict` green + APPROVED ✅ (Giovanni, 2026-07-02); marker created, NOT yet built.** GDPR erasure/anonymisation + Address entity (Module K); Round 2 P0-floor headline, size L. Confirmed: Hold-block = canon **MVP-DEC-015 `compliance`-only** (mini-ADR); J-9b export = **minimal synchronous in-memory**. Build via `./ralph.sh --change parties-anonymisation <n>` (or interactive, one task/iter). Alt floor item: RM-02 (enhanced-KYC threshold, M).
- **Overall goal:** **Round 1 ✅ cleared** (2026-07-02); now at least start **RM-01** (GDPR erasure) before Paolo.
...
- **🟡 Change authored + APPROVED (2026-07-02, marker created — NOT yet built):** OpenSpec change [`parties-anonymisation`](../../openspec/changes/parties-anonymisation/) via `/spec-to-change`. Grounded via 4 subagents (Module K PRD/Acceptance/Verdict + current code/truth-spec). Artifacts: proposal + design (D1–D7) + `party-registry` delta (**4 ADDED** — *Customer Anonymisation*, *Anonymisation Hold Precedence*, *Customer Address*, *Customer Data Export* — **+ 1 MODIFIED** *Customer Suspension and Closure*, the "anonymisation out of scope" clause → cross-ref) + **12 tasks** (groups 1–7); `openspec validate --strict` **green**.
  ...
  - **Next:** Giovanni APPROVED marker → `./ralph.sh --change parties-anonymisation <n>` (or interactive, one task/iter).
