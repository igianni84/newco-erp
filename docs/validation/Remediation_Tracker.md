...
- **Done (awaiting Giovanni review):** **RM-10** ✅ — ClubCredit issuance event `ClubCreditIssued`→`ClubCreditAccrued` (canon DEC-018) + **mini-ADR** (reverses coherent frozen-spec DEC-166; §3 "—" upgraded, Giovanni-approved). EVENT seam-name only (no event class) → zero behaviour change; suite 1767/1767, PHPStan/Pint clean. App-event→Module-S + `MembershipFeePaid` (RM-03) noted as deferred seams.
- **Active next:** **RM-24** (Product-Type immutability guard) — last Round-1 quick win before the floor builds (RM-01 erasure / RM-02 enhanced-KYC).
