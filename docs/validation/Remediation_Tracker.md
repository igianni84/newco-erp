...
- **Active next: RM-06 🟡** — OpenSpec change [`catalog-review-freshness-resubmit`](../../openspec/changes/catalog-review-freshness-resubmit/) **authored + APPROVED by Giovanni 2026-07-02** (0/10 tasks). First Round-1 item routed through `/spec-to-change` (M — touches the shared lifecycle FSM). Delivers block-gate + explicit re-submit (derive-from-audit); edit-re-arms deferred to RM-14; mini-ADR adopts canon MVP-DEC-019. Awaiting commit (push-gate) + build (ralph or interactive TDD). Closing it completes Round 1. Then Round 2 = floor builds (RM-01 erasure / RM-02 enhanced-KYC).
...
- **🟡 In progress (2026-07-02):** OpenSpec change [`catalog-review-freshness-resubmit`](../../openspec/changes/catalog-review-freshness-resubmit/) **authored + APPROVED** (proposal/spec/design/tasks; `validate --strict` green; 0/10 tasks). Delta = `MODIFIED product-catalog` *Approval Governance* (block-gate + explicit re-submit, derive-from-audit) + 4 new scenarios. Completes the prior `catalog-lifecycle-approval` design-**D5** intent (D5 imagined re-submission clearing rejection-pending, but the re-submit op was never built — `SubmitForReview` is `draft→reviewed`, un-runnable from `reviewed` — and the block-gate was never enforced). Mini-ADR `2026-07-02-adopt-dec-019-review-freshness-resubmit` (planned, task 1.1). Awaiting commit (push-gate) + build.
