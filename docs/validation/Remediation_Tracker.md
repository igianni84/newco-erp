...
- **Done (awaiting Giovanni review):** **RM-24** ✅ — Product-Type immutability guard (canon DEC-023 / BR-Identity-5) + **mini-ADR** (`2026-07-02-adopt-dec-023-product-type-immutable`; §3 "—" → "mini" — canon-DEC absent from frozen spec, same discipline as RM-04/RM-10, now the 3rd confirmation → lessons.md rule). TDD: model `updating` guard on `ProductMaster` (`isDirty('product_type')` → new `ProductTypeImmutable`), the only path-complete chokepoint (real mutable column + `$guarded=[]`, no update Action). Fires on UPDATE only (creation free), passes lifecycle transitions. Zero behaviour change — codifies an already-satisfied invariant. +2 tests; suite **1769/1769**, PHPStan/Pint clean.
