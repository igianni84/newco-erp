...
/**
 * Rejects a Composite SKU under review (§ 4.3) through the shared {@see LifecycleTransition} mechanism
 * (catalog-lifecycle-approval task 4.6; design D5; product-catalog — Requirement: Approval Governance).
 *
 ...
 * reject) and from-state guarded (a reject on a non-`reviewed` SKU throws {@see IllegalLifecycleTransition}
 * and writes nothing). A thin per-entity wrapper — the entity label `CompositeSku` matches the domain-event
 * `entity_type`; the model stays persistence-only.
 */
class RejectCompositeSkuReview
{
     ...
     */
    public function handle(CompositeSku $compositeSku, string $notes): CompositeSku
    {
        return $this->lifecycle->reject($compositeSku, 'CompositeSku', $notes);
    }
}
