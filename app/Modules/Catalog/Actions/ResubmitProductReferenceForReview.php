 ...
 *
 * After a reviewer/approver rejects a PR (it stays in `reviewed`, left REVIEW-STALE — a condition DERIVED from
 * the latest review-freshness-relevant audit verb, never a schema flag), the Creator edits in place and
 * re-submits: the PR STAYS in `reviewed`, one `audit_records` row captures the actor and the `decision:
 ...
 * non-`reviewed` PR throws {@see IllegalLifecycleTransition} and writes nothing). A thin per-entity wrapper —
 * the entity label `ProductReference` matches the domain-event `entity_type`; the model stays
 * persistence-only.
 */
