 ...
 *
 * After a reviewer/approver rejects a Master (it stays in `reviewed`, left REVIEW-STALE — a condition DERIVED
 * from the latest review-freshness-relevant audit verb, never a schema flag), the Creator edits in place and
 * re-submits: the Master STAYS in `reviewed`, one `audit_records` row captures the actor and the `decision:
 ...
 * un-re-reviewed identity edit (`UpdateProductMasterIdentity` → the `identity_updated` verb) — and a distinct
 * approver can subsequently activate. Operator-floored (a `system`/null actor cannot re-submit) and from-state
 * guarded (a re-submit on a non-`reviewed` Master throws {@see IllegalLifecycleTransition} and writes
 * nothing). A thin per-entity wrapper — the entity label `ProductMaster` matches the domain-event
