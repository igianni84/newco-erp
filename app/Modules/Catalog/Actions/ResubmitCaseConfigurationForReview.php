 ...
 *
 * After a reviewer/approver rejects a Case Configuration (it stays in `reviewed`, "rejection-pending" DERIVED
 * from the latest governance audit action — no schema flag), the Creator edits in place and re-submits: the
 * Case Configuration STAYS in `reviewed`, one `audit_records` row captures the actor and the
 * `decision: resubmitted`, and NO domain event is recorded. The re-submit is then the freshest governance
 * action, so it clears the review-freshness activation block-gate — a distinct approver can subsequently
 * activate. Operator-floored (a `system`/null actor cannot re-submit) and from-state guarded (a re-submit on a
 * non-`reviewed` Case Configuration throws {@see IllegalLifecycleTransition} and writes nothing). A thin
 * per-entity wrapper — the entity label `CaseConfiguration` matches the domain-event `entity_type`; the model
