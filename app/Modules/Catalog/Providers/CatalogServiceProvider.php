...
class CatalogServiceProvider extends ServiceProvider
{
     ...
     * this wiring `ProducerCreated`/`ProducerActivated`/`ProducerRetired` would fan
     * out zero deliveries and the producer-state projection — hence the activation
     * gate AND the Master-creation existence guard — would never update (design D4
     * risk note; catalog-module-0-completeness-sweep design D7). Registration is
     * idempotent (the registry de-dupes a repeated (event, consumer) pair); inline
     * delivery mode is the launch substrate, so no queue ADR gate is crossed.
     */
    public function boot(ConsumerRegistry $registry): void
    {
        $registry->register(ProducerLifecycleProjector::PRODUCER_CREATED, ProducerLifecycleProjector::class);
