...
/**
 * `CustomerOnboardingScreeningPassed` — recorded when a Customer's FIRST (onboarding) sanctions screening
 * completes with a `passed` verdict (parties-compliance, design L3/L4; party-registry — Requirement: Sanctions
 * Screening Events). The verbatim § 15.6 event name; one of the four sanctions screening events this slice
 * records — the ONLY compliance events in the change. KYC records none (design L3 — the § 15.1 catalog names no
 * KYC event), so there is no `CustomerKycVerified`/`ProducerKycCleared`; the future `kyc` Hold coupling is
 * Action-orchestrated, not event-driven. The four screening events split two phases × two outcomes: onboarding
 * `{Passed,Failed}` (the first screen) and rescreening `{Passed,Failed}` (every later re-screen — any
 * `trigger_source` ≠ `onboarding`). A verdict landing `under_review` is NOT a completion and records nothing
 * (design L4); a later resolution to `passed`/`failed` records the matching rescreening event.
 *
 ...
 */
final class CustomerOnboardingScreeningPassed
{
    /** The verbatim § 15.6 event name — the inter-module contract key recorded in `domain_events.name`. */
    public const NAME = 'CustomerOnboardingScreeningPassed';
