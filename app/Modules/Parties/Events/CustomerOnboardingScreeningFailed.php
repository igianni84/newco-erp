...
/**
 * `CustomerOnboardingScreeningFailed` — recorded when a Customer's FIRST (onboarding) sanctions screening
 * completes with a `failed` verdict (parties-compliance, design L3/L4; party-registry — Requirement: Sanctions
 * Screening Events). The verbatim § 15.6 event name; the `failed` outcome of the onboarding pair (its `passed`
 * sibling is {@see CustomerOnboardingScreeningPassed}). One of the four sanctions screening events this slice
 * records — the ONLY compliance events in the change (KYC records none — design L3). The four split two phases ×
 * two outcomes: onboarding `{Passed,Failed}` (the first screen) and rescreening `{Passed,Failed}` (every later
 * re-screen). A verdict landing `under_review` is NOT a completion and records nothing (design L4).
 *
 ...
 */
final class CustomerOnboardingScreeningFailed
{
    /** The verbatim § 15.6 event name — the inter-module contract key recorded in `domain_events.name`. */
    public const NAME = 'CustomerOnboardingScreeningFailed';
