 ...
 * design L3; party-registry — Requirements: Hold Lifecycle and Lift Discipline, Hold Events, Hold Registry). It
 * creates an `active` {@see Hold} carrying the {@see HoldType}, the polymorphic scope (`scope_type` + `scope_id`),
 * the optional business `reason` and the placement actor/moment, and records the verbatim {@see CustomerHoldPlaced}
 ...
 * hand) AND the path `RequireKyc` reuses to auto-place the `kyc` Hold when KYC enters `pending` (task 4.1 — one
 * Action calling another, the `RetireProducer → SunsetClub` precedent). The scope is a within-module reference with
 * NO DB FK (design L1); within-module referential integrity for the three-way polymorphic scope is the caller's (the
 * one live caller, `RequireKyc`, places on a Customer it has just transaction-locked).
 *
 ...
 *
 * Inside ONE {@see DB::transaction}: it creates the Hold (`status = active`) then records `CustomerHoldPlaced` — the
 ...
 * KYC-initiated, never a cascade step in this slice). Placing a Hold records the Hold ONLY: it performs NO status
 * transition on the scoped Customer/Account/Profile — the Hold→`suspended` coupling is a deferred demand-side seam
 * (proposal slice boundary).
 *
 * `LiftHold` (task 3.2) is the lifting counterpart; the per-type lift discipline lives on `HoldType::autoLiftable()`.
 */
class PlaceHold
{
